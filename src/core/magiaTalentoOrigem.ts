// Truques + magia sempre preparada concedidos pelo talento de Origem
// "Iniciado em Magia" (Acólito/Guia/Sábio) — diferente de
// `magiasEspecie.ts` (gatilho é nível de PERSONAGEM, cresce com o
// tempo), a escolha aqui é feita 1x na criação
// (`TalentoOrigemEscolhasStep`) e fica fixa depois — este talento não
// ganha truque/magia extra por nível.

import type { WizardSelection } from './personagem';

export function truquesMagiaIniciada(selection: WizardSelection): string[] {
  return selection.truquesMagiaIniciadaEscolhidos;
}

export function magiasMagiaIniciada(selection: WizardSelection): string[] {
  return selection.magiaMagiaIniciadaEscolhida ? [selection.magiaMagiaIniciadaEscolhida] : [];
}
