// Gerado a partir de dnd-master-referencia.xlsx, aba "Magias". Não editar
// valores à mão. Linhas de cabeçalho de círculo (sem nome de magia)
// filtradas. NÃO inclui a aba "Magias — UA Psion (não-oficial)" —
// conteúdo não-oficial, fora de escopo.
//
...
// Conhecimento pode preparar magia de Clérigo/Druida/Mago que não tem
// "Bardo" nessa lista; isso é resolvido em tempo de leitura pela
// característica específica, não duplicado aqui).
//
...
//
// `upcastTipo`/`upcastCirculoBase`/`upcastDado`/`upcastFlat`/
...
// PENDENCIAS.md "Motor de rolagem de dano de Magia"). Extração
// concluída (122/390 magias têm dano direto num alvo, cruzado com os
// PDFs do Cap. 7 do Livro do Jogador — mesmo processo do Upcast);
// `null` = a magia não causa dano direto num alvo (inclui dano que só
// atinge o PRÓPRIO conjurador, ex.: Desejo/Contato Extraplanar, e dano
// condicional futuro sem ataque/salvaguarda no momento, ex.: Missão —
// ver `DECISOES-DADOS.md`). `danoBaseTipo: "escolhido"` = o tipo de
// dano varia por escolha do jogador ao conjurar (ex.: Orbe Cromático)
// — o dado é fixo, só o tipo muda; `"aleatório"` = o tipo varia por
// sorteio da própria magia, não escolha do jogador (ex.: Muralha
// Prismática, cada camada tem uma cor/tipo diferente). Magia com 2+
// efeitos de dano distintos (ex.: Faca de Gelo: 1d10 Perfurante no
// acerto + 2d6 Gélido na explosão) guarda só o valor que o Upcast
// realmente escala (conferido linha a linha contra `upcastTexto`) —
// o outro efeito é fixo e não precisa de campo próprio, já aparece em
// `descricaoCurta`. Magia com dano escalonado por turno demais pra um
// campo único (ex.: Tempestade da Vingança) também fica `null`.
//
// `escalaTruqueTipo` cobre "Aprimoramento de Truque" — quase todo
...
// salvaguarda varia por sorteio, mesmo caso das magias Prismáticas) |
// `null` (a magia não usa nem ataque nem salvaguarda — a maioria das
// magias sem dano, ex.: buffs/utilidade, mas também algumas COM dano
// automático, ex.: Palavra de Poder: Matar). Extraído do mesmo texto
// de `danoBaseDado`/Upcast (frases "ataque mágico à distância/corpo a
// corpo" e "salvaguarda de <atributo>" no PDF), sem precisar de coluna
...
/** 'dado' = truque com "Aprimoramento de Truque" que soma 1 dado do
 * mesmo tamanho de `danoBaseDado` por nível 5/11/17 do PERSONAGEM (não
 * do círculo gasto, que truque não tem) — ver comentário acima. */
export type EscalaTruqueTipo = 'dado';
...
export interface Magia {
  ...
  fonte: string;
  upcastTipo: UpcastTipo | null;
  upcastCirculoBase: number | null;
  upcastDado: string | null;
  upcastFlat: number | null;
  upcastAlvos: number | null;
  upcastTexto: string | null;
  danoBaseDado: string | null;
  danoBaseTipo: string | null;
  /** Dado ALTERNATIVO ao `danoBaseDado`, usado em vez dele quando uma
   * condição no alvo (que o app não rastreia, ex.: "alvo já perdeu
   * algum PV") se aplica — hoje só Badalar Fúnebre ("1d8" normal, mas
   * "1d12" se o alvo já estiver ferido; escala com Aprimoramento de
   * Truque igual ao dado normal, mesmo `escalaTruqueTipo`/Upcast).
   * Auditoria de 2026-09 nas 391 magias confirmou que é a ÚNICA com
   * esse padrão — se aparecer outra, este campo já serve, sem precisar
   * de um 3º dado. UI mostra os 2 botões de "Rolar Dano" lado a lado
   * (`danoCondicionalTexto` explica a condição), o jogador decide qual
   * bate com a cena. `null` = a magia não tem dado alternativo. */
  danoCondicionalDado: string | null;
  /** Texto curto explicando a condição do `danoCondicionalDado` (ex.:
   * "alvo já ferido") — mostrado no botão alternativo de "Rolar Dano".
   * `null` sempre que `danoCondicionalDado` também for `null`. */
  danoCondicionalTexto: string | null;
  /** Dado + mod. fixo de cura no círculo/nível mínimo da magia (ex.:
   * Palavra Curativa = "2d4"; Regeneração = "4d8 + 15") — mesmo
   * formato de `danoBaseDado` ("NdM" ou "NdM + F"), reaproveita o
   * MESMO Upcast estruturado (`upcastTipo`/`upcastDado`/...) já
   * extraído pra toda magia (ver comentário do Upcast acima — ele
   * descreve como QUALQUER efeito escala, não só dano, por isso já
   * vinha preenchido mesmo pra magia de cura antes deste campo
   * existir). `null` = a magia não cura com um valor de dado único e
   * simples (cura de valor fixo tipo "recupera todos os PV" ou "70
   * PV", cura derivada de outro efeito como Toque Vampírico, ou cura
   * variável demais pra um campo único como Vigor Arcano) — coberta
   * só quando o Osmar confirmar caso a caso. Extração de 2026-09:
   * só 7 das 391 magias têm cura em formato de dado único (conferido
   * contra `Descrição Completa` uma por uma, cruzado com o Upcast já
   * existente pra garantir automático — ver DECISOES-DADOS.md). */
  curaBaseDado: string | null;
  escalaTruqueTipo: EscalaTruqueTipo | null;
...
export const magias: Magia[] = [
  {
    ...
    componentes: "V, S",
    duracao: "Instantânea",
    descricaoCompleta: "Escolha uma criatura no alcance da magia que tenha 0 Pontos de Vida e não esteja morta. A criatura fica Estável. Aprimoramento de Truque. O alcance da magia dobra quando você atinge os níveis 5 (9 metros), 11 (18 metros) e 17 (36 metros).",
    descricaoCurta: "Você estabiliza à distância uma criatura caída a 0 PV (sem curar). Alcance dobra nos níveis 5/11/17.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
    upcastFlat: null,
    upcastAlvos: null,
    upcastTexto: null,
    danoBaseDado: null,
    danoBaseTipo: null,
    danoCondicionalDado: null,
    danoCondicionalTexto: null,
    curaBaseDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Você emana um sentimento mágico de amizade. Salv. Sabedoria ou fica Enfeitiçado pela duração (falha automática se não for Humanoide, já estiver lutando com você, ou já sofreu essa magia sua nas últimas 24h). Encerra se sofrer dano ou você atacar/causar dano/forçar salvaguarda. Ao acabar, o alvo sabe que foi enfeitiçado — risco de criar um inimigo.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "Instantânea",
    descricaoCompleta: "Sussurrando para os espíritos da natureza, você cria um dos seguintes efeitos no alcance da magia. Brincar com Fogo. Você acende ou apaga uma vela, uma tocha ou uma fogueira. Efeito Sensorial. Você cria um efeito sensorial inofensivo, como folhas caindo, fadas dançantes espectrais, uma brisa suave, o som de um animal ou o leve odor de gambá. O efeito deve caber em um Cubo de 1,5 metro de lados. Um sprite conjura Arte Druídica para fazer as flores florescerem. Florescimento. Você instantaneamente faz uma flor desabrochar, uma vagem se abrir ou um botão de folha se abrir. Sensor Climático. Você cria um efeito sensorial minúsculo e inofensivo que prevê qual será o clima no local onde você está pelas próximas 24 horas. O efeito pode se manifestar como uma esfera dourada para céu claro, uma nuvem para chuva, flocos de neve caindo para neve e assim por diante. Esse efeito persiste por 1 rodada.",
    descricaoCurta: "Sussurra aos espíritos da natureza pra um efeito pequeno à escolha: acender/apagar chama; efeito sensorial inofensivo (folhas, brisa, som, cheiro); flor desabrochando; ou previsão do clima nas próximas 24h (efeito visual dura 1 rodada).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    danoCondicionalDado: "1d12",
    danoCondicionalTexto: "alvo já ferido",
    curaBaseDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Uma bolha de ácido explode numa esfera de 1,5m. Salv. Destreza (área) ou 1d6 dano Ácido. Escala 5/11/17.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "Instantânea",
    descricaoCompleta: "Você exerce controle sobre os elementos, criando um dos seguintes efeitos no alcance da magia. Convocar Água. Você cria um jato de névoa fria que umedece levemente criaturas e objetos em um Cubo de 1,5 metro de lado. Como alternativa, você cria 1 xícara de água limpa em um recipiente aberto ou em uma superfície, e a água evapora em 1 minuto. Convocar Brisa. Você cria uma brisa forte o suficiente para ondular um tecido, mexer poeira, farfalhar folhas e fechar portas e persianas abertas, tudo em um Cubo de 1,5 metro de lado. Portas e persianas que estiverem sendo mantidas abertas por alguém ou alguma força não são afetadas. Convocar Fogo. Você cria uma fina nuvem de brasas inofensivas e fumaça colorida e perfumada em um Cubo de 1,5 metro de lados. Você escolhe a cor e o perfume, e as brasas podem acender velas, tochas ou lâmpadas nessa área. O cheiro da fumaça permanece por 1 minuto. Convocar Terra. Você cria uma fina camada de poeira ou areia que cobre superfícies em uma área quadrada de 1,5 metro de lado, ou faz com que uma única palavra apareça em sua caligrafia em um pedaço de terra ou areia. Esculpir Elemento. Você faz com que terra, areia, fogo, fumaça, névoa ou água que caiba em um Cubo de 30 centímetros de lado assuma uma forma rudimentar (como a de uma criatura) por 1 hora. Um draconato Druida usa Emaranhar para conter gnolls em frenesi.",
    descricaoCurta: "Controle sutil sobre os elementos — escolha: névoa d'água leve ou 1 xícara de água limpa; brisa que mexe objetos leves; fumaça colorida/perfumada que acende pavios; poeira/areia decorativa ou escrita; ou esculpir água/terra/fumaça numa forma rudimentar por 1h. Puro utilidade, sem dano.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "Instantânea",
    descricaoCompleta: "Você lança uma faísca de luz em uma criatura ou objeto no alcance da magia. Realize um ataque mágico à distância contra o alvo. Em caso de acerto, o alvo sofre 1d8 pontos de dano Radiante e, até o final do seu próximo turno, emite Meia-luz em um raio de 3 metros e não pode se beneficiar da condição Invisível. Aprimoramento de Truque. O dano aumenta em 1d8 quando você atinge os níveis 5 (2d8), 11 (3d8) e 17 (4d8).",
    descricaoCurta: "Faísca de luz à distância. Ataque: 1d8 dano Radiante; alvo emite Meia-luz 3m e perde benefício de Invisível até o fim do seu próximo turno. Escala 5/11/17.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    duracao: "1 hora",
    descricaoCompleta: "Você toca em um objeto Grande ou menor que não está sendo usado ou carregado por outra pessoa. Até que a magia termine, o objeto emite Luz Plena em um raio de 6 metros e Meia-luz por mais 6 metros. A luz pode ter a cor que você desejar. Cobrir o objeto com algo opaco bloqueia a luz. A magia se encerra se você a conjurar novamente. Um elfo Clérigo usa a magia Luz do Dia para trazer a luz do amanhecer para uma corte de vampiros.",
    descricaoCurta: "Toca um objeto (Grande ou menor); ele emite Luz Plena 6m + Meia-luz 6m, cor à escolha, até cobrir com algo opaco ou reconjurar.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "1 minuto",
    descricaoCompleta: "Uma mão espectral flutuante aparece em um ponto que você escolher no alcance da magia. A mão permanece pela duração. A mão desaparece se estiver a mais de 9 metros de você ou se você conjurar esta magia novamente. Ao conjurar a magia, você pode usar a mão para manipular um objeto, abrir uma porta ou um recipiente destrancado, guardar ou recolher itens de recipientes abertos, ou despejar o conteúdo de um frasco. Como uma ação Usar Magia em seus próximos turnos, você pode controlar a mão novamente. Como parte dessa ação, você pode mover a mão até 9 metros. A mão não pode atacar, ativar itens mágicos ou carregar mais de 5 quilos.",
    descricaoCurta: "Mão espectral manipula objetos, abre portas/recipientes destrancados, guarda ou pega itens, despeja frascos — até 5kg, sem atacar. Some se afastar 9m ou reconjurar; controla de novo com Ação de Usar Magia.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "Concentração, até 1 minuto",
    descricaoCompleta: "Você toca uma criatura voluntária e escolhe uma perícia. Até que a magia termine, a criatura adiciona 1d4 a qualquer teste de atributo usando a perícia escolhida.",
    descricaoCurta: "Toca uma criatura voluntária; ela soma 1d4 num teste de perícia à escolha, enquanto durar.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    duracao: "Instantânea",
    descricaoCompleta: "Uma radiância ardente irrompe de você em uma Emanação de 1,5 metro. Cada criatura à sua escolha à sua vista nela deve ser bem-sucedida em uma salvaguarda de Constituição ou sofre 1d6 pontos de dano Radiante. Aprimoramento de Truque. O dano aumenta em 1d6 quando você atinge os níveis 5 (2d6), 11 (3d6) e 17 (4d6).",
    descricaoCurta: "Explosão de luz ardente ao seu redor (1,5m). Cada criatura escolhida à vista: Salv. Constituição ou 1d6 dano Radiante. Escala 5/11/17.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "Até 1 hora",
    descricaoCompleta: "Você cria um efeito mágico no alcance da magia. Escolha o efeito entre as opções abaixo. Se conjurar esta magia várias vezes, você pode ter até três de seus efeitos não instantâneos ativos ao mesmo tempo. Brincar com Fogo. Você instantaneamente acende ou apaga uma vela, uma tocha ou uma pequena fogueira. Criação Menor. Você cria uma bugiganga não mágica, sem valor monetário e incapaz de causar dano, ou uma imagem ilusória, sendo que ambas devem caber na sua mão. Ela dura até o final do seu próximo turno. Efeito Sensorial. Você cria um efeito sensorial instantâneo e inofensivo, como uma chuva de faíscas, um sopro de vento, notas musicais suaves ou um odor estranho. Limpar ou Sujar. Você limpa ou suja instantaneamente um objeto com menos de 30 centímetros cúbicos. Marca Mágica. Você faz aparecer uma cor, uma pequena marca ou um símbolo em um objeto ou superfície por 1 hora. Sensação Ideal. Você esfria, aquece ou dá sabor a um material não vivo com menos de 30 centímetros cúbicos por 1 hora.",
    descricaoCurta: "Pequeno truque mágico à escolha: acender/apagar chama; bugiganga sem valor ou imagem ilusória na mão (até seu próximo turno); efeito sensorial instantâneo inofensivo; limpar/sujar objeto pequeno; marca/símbolo colorido por 1h; esquentar/esfriar/temperar material pequeno por 1h. Até 3 efeitos duradouros simultâneos.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "Concentração, até 1 minuto",
    descricaoCompleta: "Sempre que uma criatura realizar uma jogada de ataque contra você antes que a magia termine, o atacante subtrai 1d4 da jogada de ataque.",
    descricaoCurta: "Enquanto durar, qualquer ataque contra você tem -1d4 na jogada.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Partícula de fogo dispara da mão. Ataque à distância: 1d10 dano Ígneo; incendeia objeto inflamável desprotegido. Escala 5(2d10)/11(3d10)/17(4d10).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "Instantânea",
    descricaoCompleta: "Você projeta uma névoa tóxica em uma criatura no alcance da magia. Realize um ataque mágico à distância contra o alvo. Em caso de acerto, o alvo sofre 1d12 pontos de dano Venenoso. Aprimoramento de Truque. O dano aumenta em 1d12 quando você atinge os níveis 5 (2d12), 11 (3d12) e 17 (4d12).",
    descricaoCurta: "Névoa tóxica projetada à distância. Ataque: 1d12 dano Venenoso. Escala 5/11/17.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    duracao: "Instantânea",
    descricaoCompleta: "Magias como Caminhar Sobre as Águas e Respirar na Água auxiliam viagens a reinos aquáticos. Essa magia repara uma única ruptura ou rasgo em um objeto que você tocar, como um elo de corrente quebrado, partes de uma chave partida, uma capa rasgada ou um odre furado. Se a ruptura ou rasgo não ultrapassar 30 centímetros em qualquer dimensão, ela é reparada sem deixar vestígios do dano anterior. Essa magia pode reparar fisicamente um item mágico, mas não pode restaurar a magia de tal objeto.",
    descricaoCurta: "Conserta uma quebra/rasgo pequeno (até 30cm) num objeto tocado, sem deixar vestígio — inclusive item mágico fisicamente, mas não restaura a magia dele.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "Instantânea",
    descricaoCompleta: "Raios saltam de você em direção a uma criatura que você tenta tocar. Realize uma jogada de ataque mágico corpo a corpo contra o alvo. Em caso de acerto, o alvo sofre 1d8 pontos de dano Elétrico e não pode realizar Ataques de Oportunidade até o início do próximo turno dele. Aprimoramento de Truque. O dano aumenta em 1d8 quando você atinge os níveis 5 (2d8), 11 (3d8) e 17 (4d8).",
    descricaoCurta: "Raios saltam da sua mão. Ataque corpo a corpo: 1d8 dano Elétrico; alvo não pode fazer Ataque de Oportunidade até o início do próximo turno dele. Escala 5/11/17.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "Instantânea",
    descricaoCompleta: "Canalizando o frio da sepultura, realize um ataque mágico corpo a corpo contra um alvo no alcance da magia. Em caso de acerto, o alvo sofre 1d10 pontos de dano Necrótico e não pode recuperar Pontos de Vida até o final do seu próximo turno. Aprimoramento de Truque. O dano aumenta em 1d10 quando você atinge os níveis 5 (2d10), 11 (3d10) e 17 (4d10).",
    descricaoCurta: "Frio da sepultura num toque. Ataque corpo a corpo: 1d10 dano Necrótico; alvo não recupera PV até o fim do próximo turno dele. Escala 5/11/17.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "S",
    duracao: "Instantânea",
    descricaoCompleta: "Cada criatura em uma Emanação de 1,5 metro originada em você deve ser bem-sucedida em uma salvaguarda de Constituição ou sofre 1d6 pontos de dano Trovejante. O som estrondoso da magia pode ser ouvido a até 30 metros de distância. Aprimoramento de Truque. O dano aumenta em 1d6 quando você atinge os níveis 5 (2d6), 11 (3d6) e 17 (4d6).",
    descricaoCurta: "Estrondo sônico ao seu redor (1,5m), audível a 30m. Cada criatura na área: Salv. Constituição ou 1d6 dano Trovejante. Escala 5/11/17.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V",
    duracao: "Instantânea",
    descricaoCompleta: "Você libera uma série de insultos carregados com encantamentos sutis em uma criatura que pode ouvir você e que esteja à sua vista e no alcance da magia. O alvo deve ser bem-sucedido em uma salvaguarda de Sabedoria ou sofre 1d6 pontos de dano Psíquico e tem Desvantagem na próxima jogada de ataque que realizar antes do final do próximo turno dele. Aprimoramento de Truque. O dano aumenta em 1d6 quando você atinge os níveis 5 (2d6), 11 (3d6) e 17 (4d6).",
    descricaoCurta: "Insultos com encantamento sutil. Salv. Sabedoria ou 1d6 dano Psíquico e Desvantagem no próximo ataque do alvo antes do fim do turno seguinte dele. Escala 5/11/17.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Define um alarme de intrusão numa porta/janela/área (até cubo de 6m). Escolha: alarme mental (acorda você a até 1,5km) ou sonoro (sino audível 18m por 10s). Pode isentar criaturas específicas.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    duracao: "24 horas",
    descricaoCompleta: "Escolha como alvo uma Fera à sua vista e no alcance da magia. O alvo deve ser bem-sucedido em uma salvaguarda de Sabedoria ou tem a condição Enfeitiçado pela duração da magia. Se você ou um de seus aliados causar dano ao alvo, a magia encerra. Usando um Espaço de Magia de Círculo Superior. Você pode escolher uma criatura adicional para cada círculo de espaço de magia acima de 1.",
    descricaoCurta: "Alimenta uma Fera com comida na mão. Salv. Sabedoria ou fica Enfeitiçada por 24h; encerra se você/aliado causar dano nela. Upcast: +1 alvo por círculo.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "alvo-por-circulo",
    upcastCirculoBase: 1,
    upcastDado: null,
    upcastFlat: null,
    upcastAlvos: 1,
  ...
  },
  {
    ...
    duracao: "8 horas",
    descricaoCompleta: "Você toca uma criatura voluntária que não está usando armadura. Até que a magia termine, a CA base do alvo se torna 13 mais o modificador de Destreza dele. A magia se encerra se o alvo vestir uma armadura.",
    descricaoCurta: "Toca criatura sem armadura; CA base vira 13+Destreza pela duração (8h). Encerra se vestir armadura.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    duracao: "1 hora",
    descricaoCompleta: "Um frio mágico protetor envolve você. Você recebe 5 Pontos de Vida Temporários. Se uma criatura acertar você com uma jogada de ataque corpo a corpo antes que a magia termine, a criatura sofre 5 pontos de dano Gélido. A magia encerra se você não tiver Pontos de Vida Temporários. Usando um Espaço de Magia de Círculo Superior. Os Pontos de Vida Temporários e o dano Gélido aumentam em 5 para cada círculo de espaço de magia acima de 1.",
    descricaoCurta: "Frio mágico te envolve: +5 PV temporários; quem te acertar corpo a corpo sofre 5 dano Gélido. Encerra ao perder os PV temp. Upcast: +5 PV temp e +5 dano por círculo.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "flat-por-circulo",
    upcastCirculoBase: 1,
    upcastDado: null,
    upcastFlat: 5,
    upcastAlvos: null,
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "Instantânea",
    descricaoCompleta: "Clamando por Hadar, você faz com que tentáculos irrompam em você. Cada criatura em uma Emanação de 3 metros originada em você realiza uma salvaguarda de Força. Se falhar, um alvo sofre 2d6 pontos de dano Necrótico e não pode executar Reações até o início do próximo turno dele. Em caso de sucesso, um alvo sofre apenas metade do dano. Usando um Espaço de Magia de Círculo Superior. O dano aumenta em 1d6 pontos para cada círculo de espaço de magia acima de 1. Um Golias Bruxo convoca a Armadura de Agathys e os Braços de Hadar para combater inimigos das profundezas.",
    descricaoCurta: "Tentáculos irrompem de você (emanação 3m). Salv. Força ou 2d6 dano Necrótico + sem Reação até o próximo turno do alvo (sucesso=metade do dano). Upcast: +1d6.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
  ...
  },
  {
    ...
    descricaoCurta: "Abençoa até 3 criaturas; somam 1d4 em ataques e salvaguardas, enquanto durar. Upcast: +1 alvo/círculo.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "alvo-por-circulo",
    upcastCirculoBase: 1,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Entende qualquer idioma ouvido ou escrito que toque, pela duração. Não decodifica códigos/mensagens secretas.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "Instantânea",
    descricaoCompleta: "Uma criatura que você toca recupera um número de Pontos de Vida igual a 2d8 mais seu modificador de atributo de conjuração. Usando um Espaço de Magia de Círculo Superior. A cura aumenta em 2d8 para cada círculo de espaço de magia acima de 1.",
    descricaoCurta: "Toca e cura 2d8 + mod. de conjuração PV. Upcast: +2d8 por círculo.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "dado-por-circulo",
    upcastCirculoBase: 1,
    upcastDado: "2d8",
  ...
  },
  {
    ...
    componentes: "V",
    duracao: "Instantânea",
    descricaoCompleta: "O alvo sofre 2d8 pontos de dano Radiante adicionais do ataque. O dano aumenta em 1d8 se o alvo for um Ínfero ou um Morto-vivo. Usando um Espaço de Magia de Círculo Superior. O dano aumenta em 1d8 para cada círculo de espaço de magia acima de 1.",
    descricaoCurta: "Ação Bônus após acertar: +2d8 dano Radiante (+1d8 extra se o alvo for Ínfero/Morto-vivo). Upcast: +1d8.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "dado-por-circulo",
    upcastCirculoBase: 1,
    upcastDado: "1d8",
  ...
  },
  {
    ...
    componentes: "V",
    duracao: "Instantânea",
    descricaoCompleta: "Seu ataque soa como um trovão audível a até 90 metros de você, e o alvo sofre 2d6 pontos de dano Trovejante adicionais do ataque. Além disso, se o alvo Uma anã Paladina fortalece sua arma com Destruição Divina, desencadeando a ira sagrada em ínferos Mezzoloths. for uma criatura, ele deve ser bem-sucedido em uma salvaguarda de Força ou é empurrado 3 metros para longe de você e tem a condição Caído. Usando um Espaço de Magia de Círculo Superior. O dano aumenta em 1d6 pontos para cada círculo de espaço de magia acima de 1.",
    descricaoCurta: "Ação Bônus após acertar: +2d6 dano Trovejante (som audível 90m); Salv. Força ou empurra 3m e Caído. Upcast: +1d6.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "dado-por-circulo",
    upcastCirculoBase: 1,
    upcastDado: "1d6",
  ...
  },
  {
    ...
    circulo: 1,
    escola: "Adivinhação",
    classes: ["Bardo", "Bruxo", "Clérigo", "Druida", "Feiticeiro", "Guardião", "Mago", "Paladino"],
    tempoConjuracao: "Ação ou Ritual",
    alcance: "Pessoal",
    componentes: "V, S",
    ...
    descricaoCurta: "Sente magia num raio de 9m; pode ver aura e escola da magia. Bloqueada por pedra/metal/chumbo.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Sente venenos, criaturas venenosas e doenças mágicas num raio de 9m, identificando o tipo. Bloqueado por pedra/metal/chumbo.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Sente Aberrações/Celestiais/Elementais/Feéricos/Ínferos/Mortos-Vivos e áreas com Consagrar, num raio de 9m. Bloqueado por pedra/metal/chumbo.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "1 minuto ou Ritual",
    alcance: "Toque",
    componentes: "S, M (tinta no valor de 10 ou mais PO, que a magia consome)",
    duracao: "10 dias",
    descricaoCompleta: "Você escreve em pergaminho, papel ou outro material apropriado e o imbuí com uma ilusão que permanece pela duração. Para você e quaisquer criaturas que escolher ao conjurar a magia, a escrita parece normal, como se estivesse escrita por sua mão, e transmite o significado de sua intenção no momento em que o texto foi redigido. Para os demais, as palavras aparecem como se estivessem em um alfabeto desconhecido ou mágico, tornando-se ininteligíveis. Se preferir, a ilusão pode alterar o significado, a caligrafia e o idioma do texto, embora você deva conhecer o idioma. Se a magia for dissipada, o texto original e a ilusão desaparecem. Uma criatura que possui Visão Verdadeira pode ler a mensagem oculta.",
    descricaoCurta: "Texto escrito parece dizer uma coisa pra quem você escolher, e outra (ou ilegível) pra qualquer outro — pode até mudar idioma/caligrafia. Dura 10 dias. Visão Verdadeira revela a mensagem real.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    escola: "Abjuração",
    classes: ["Feiticeiro", "Mago"],
    tempoConjuracao: "Reação, que você executa quando é atingido por uma jogada de ataque ou é alvo da magia Mísseis Mágicos",
    alcance: "Pessoal",
    componentes: "V, S",
    duracao: "1 rodada",
    descricaoCompleta: "Uma barreira imperceptível de energia mágica o protege. Até o início do seu próximo turno, você recebe um bônus de +5 na CA, incluindo contra o ataque que ativou a magia, e não sofre dano de Mísseis Mágicos.",
    descricaoCurta: "Reação ao ser atingido ou alvo de Mísseis Mágicos: +5 CA até o início do próximo turno (vale contra o ataque que ativou); anula dano de Mísseis Mágicos.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    duracao: "Concentração, até 10 minutos",
    descricaoCompleta: "Um campo cintilante envolve uma criatura à sua escolha no alcance da magia, concedendo um bônus de +2 na CA pela duração da magia.",
    descricaoCurta: "+2 CA numa criatura à distância, enquanto durar (até 10 min).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "10 minutos",
    descricaoCompleta: "Pela duração da magia, você pode compreender e se comunicar verbalmente com Feras, e pode usar com elas qualquer uma das opções de perícia da ação Influenciar. A maioria das Feras oferece pouca informação sobre assuntos que não estejam diretamente ligados à sobrevivência ou ao companheirismo mas, no mínimo, uma Fera pode fornecer informações sobre áreas vizinhas e monstros, incluindo o que percebeu no último dia. Um Druida humano usa Falar com Animais para conversar com seu melhor amigo urso.",
    descricaoCurta: "Compreende e conversa com Feras por 10 min; pode usar ação Influenciar nelas. A maioria só sabe sobre sobrevivência/território, mas pode informar sobre a área e o que viu no último dia.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "1 minuto",
    descricaoCompleta: "Até que a magia termine, seus ataques com armas causam 1d4 pontos de dano Radiante adicionais em um acerto.",
    descricaoCurta: "Seus ataques com arma causam +1d4 dano Radiante, por 1 min.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V",
    duracao: "Concentração, até 1 minuto",
    descricaoCompleta: "Objetos em um Cubo de 6 metros de lado no alcance da magia são delineados por luz azul, verde ou violeta (à sua escolha). Cada criatura no Cubo também é delineada se falhar em uma salvaguarda de Destreza. Pela duração da magia, objetos e criaturas afetadas emitem Meia-Luz em um raio de 3 metros e não podem se beneficiar da condição Invisível. Jogadas de ataque contra uma criatura ou objeto afetado têm Vantagem se o atacante puder vê-la.",
    descricaoCurta: "Delineia objetos num cubo 6m com luz colorida; criaturas na área: Salv. Destreza ou também delineadas — emitem Meia-luz 3m e perdem Invisibilidade. Ataques contra alvos delineados têm Vantagem.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    escola: "Invocação",
    classes: ["Guardião"],
    tempoConjuracao: "Ação Bônus, que você realiza imediatamente após atingir uma criatura com uma arma",
    alcance: "Pessoal",
    componentes: "V",
    duracao: "Concentração, até 1 minuto",
    ...
    descricaoCurta: "Ação Bônus após acertar com arma: vinhas prendem o alvo — Salv. Força (Vantagem se Grande+) ou Contido; sofre 1d6 Perfurante no início de cada turno preso; pode tentar Atletismo pra escapar. Upcast: +1d6.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "dado-por-circulo",
    upcastCirculoBase: 1,
    upcastDado: "1d6",
  ...
  },
  {
    ...
    tempoConjuracao: "Ação",
    alcance: "18 metros",
    componentes: "V, S, M (um pouco de banha de porco ou manteiga)",
    duracao: "1 minuto",
    descricaoCompleta: "Uma graxa não inflamável cobre o solo em um quadrado de 3 metros de lados centrado em um ponto no alcance da magia e o transforma em Terreno Difícil pela duração da magia. Quando a graxa aparece, cada criatura em pé e na área da magia deve ser bem-sucedida em uma salvaguarda de Destreza ou tem a condição Caído. Uma criatura que entra na área ou termina o turno nela também deve ser bem-sucedida nessa salvaguarda ou fica Caída.",
    descricaoCurta: "Cobre um quadrado de 3m com graxa (Terreno Difícil, 1 min). Quem estiver/entrar/terminar turno lá: Salv. Destreza ou Caído.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "Instantânea",
    descricaoCompleta: "Uma criatura que você tocar realiza uma salvaguarda de Constituição, sofrendo 2d10 pontos de dano Necrótico se falhar ou metade desse dano em caso de sucesso. Usando um Espaço de Magia de Círculo Superior. O dano aumenta em 1d10 para cada círculo de espaço de magia acima de 1.",
    descricaoCurta: "Toque: Salv. Constituição ou 2d10 dano Necrótico (sucesso=metade). Upcast: +1d10.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "dado-por-circulo",
    upcastCirculoBase: 1,
    upcastDado: "1d10",
  ...
  },
  {
    ...
    duracao: "Instantânea",
    descricaoCompleta: "Você lança uma série deslumbrante de luzes ofuscantes e coloridas. Cada criatura em um Cone de 4,5 metros originado em você deve ser bem-sucedida em uma salvaguarda de Constituição ou tem a condição Cego até o final do seu próximo turno.",
    descricaoCurta: "Luzes ofuscantes coloridas num Cone de 4,5m. Salv. Constituição ou Cego até o fim do seu próximo turno.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Marca 1 criatura como presa: +1d6 dano Energético em acertos nela + Vantagem em Percepção/Sobrevivência pra rastreá-la. Se ela morre, pode remarcar outra (Ação Bônus). Upcast: Concentração dura mais (3-4º:8h / 5º+:24h).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "outro",
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "Instantânea",
    descricaoCompleta: "Uma fina camada de chamas parte de você. Cada criatura em um Cone de 4,5 metros realiza uma salvaguarda de Destreza, sofrendo 3d6 pontos de dano Ígneo se falhar, ou metade desse dano em caso de sucesso. Objetos inflamáveis no Cone que não estão sendo usados ou carregados entram em combustão. Usando um Espaço de Magia de Círculo Superior. O dano aumenta em 1d6 pontos para cada círculo de espaço de magia acima de 1.",
    descricaoCurta: "Chamas num Cone de 4,5m. Salv. Destreza ou 3d6 dano Ígneo (sucesso=metade); incendeia inflamáveis desprotegidos. Upcast: +1d6.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "dado-por-circulo",
    upcastCirculoBase: 1,
    upcastDado: "1d6",
  ...
  },
  {
    ...
    descricaoCurta: "Onda sônica ao seu redor (cubo 4,5m), audível 90m. Salv. Constituição ou 2d8 dano Trovejante + empurra 3m (sucesso=metade dano, sem empurrão); objetos soltos também são empurrados. Upcast: +1d8.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "dado-por-circulo",
    upcastCirculoBase: 1,
    upcastDado: "1d8",
  ...
  },
  {
    ...
    componentes: "V",
    duracao: "Instantânea",
    descricaoCompleta: "Uma criatura à sua escolha, à sua vista e no alcance da magia recupera Pontos de Vida iguais a 2d4 mais seu modificador de atributo de conjuração. Usando um Espaço de Magia de Círculo Superior. A cura aumenta em 2d4 para cada círculo de espaço de magia acima de 1.",
    descricaoCurta: "Ação Bônus à distância: cura 2d4 + mod. de conjuração PV. Upcast: +2d4.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "dado-por-circulo",
    upcastCirculoBase: 1,
    upcastDado: "2d4",
  ...
  },
  {
    ...
    duracao: "1 hora",
    descricaoCompleta: "Você toca uma criatura. O Deslocamento do alvo aumenta em 3 metros até que a magia termine. Usando um Espaço de Magia de Círculo Superior. Você pode escolher uma criatura adicional para cada círculo de espaço de magia acima de 1.",
    descricaoCurta: "Toca; +3m de Deslocamento por 1h. Upcast: +1 alvo/círculo.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "alvo-por-circulo",
    upcastCirculoBase: 1,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "Instantânea",
    descricaoCompleta: "Você remove veneno e podridão de alimentos e bebidas não mágicos em uma Esfera de 1,5 metro de raio centrada em um ponto no alcance da magia. Um pequenino Bruxo atinge um zumbi com o arco elétrico do Raio de Bruxa.",
    descricaoCurta: "Remove veneno e podridão de comida/bebida não-mágica numa esfera de 1,5m.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    escola: "Transmutação",
    classes: ["Bardo", "Feiticeiro", "Mago"],
    tempoConjuracao: "Reação, que você executa quando você ou uma criatura à sua vista a até 18 metros de você entra em queda",
    alcance: "18 metros",
    componentes: "V, M (uma pequena pena ou pedaço de penugem)",
    duracao: "1 minuto",
    descricaoCompleta: "Escolha até cinco criaturas em queda no alcance da magia. A taxa de queda de cada uma diminui para 18 metros por rodada até que a magia termine. Se uma criatura pousar antes do fim da magia, não sofre dano da queda e a magia se encerra para ela.",
    descricaoCurta: "Reação a uma queda (sua ou de até 5 criaturas à vista, 18m): reduz a velocidade de queda pra 18m/rodada; quem pousa antes do fim não sofre dano.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "1 rodada",
    descricaoCompleta: "Você lança um raio de luz em direção a uma criatura no alcance da magia. Realize um ataque mágico à distância contra o alvo. Em caso de acerto, ele sofre 4d6 pontos de dano Radiante, e a próxima jogada de ataque realizado contra ele antes do final do seu próximo turno tem Vantagem. Usando um Espaço de Magia de Círculo Superior. O dano aumenta em 1d6 pontos para cada círculo de espaço de magia acima de 1.",
    descricaoCurta: "Raio de luz à distância. Ataque: 4d6 dano Radiante; próximo ataque contra o alvo (até seu próximo turno) tem Vantagem. Upcast: +1d6.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "dado-por-circulo",
    upcastCirculoBase: 1,
    upcastDado: "1d6",
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "Instantânea",
    descricaoCompleta: "Você dispara um raio esverdeado em uma criatura no alcance da magia. Realize um ataque mágico à distância contra o alvo. Em caso de acerto, o alvo sofre 2d8 pontos de dano Venenoso e tem a condição Envenenado até o final do seu próximo turno. Usando um Espaço de Magia de Círculo Superior. O dano aumenta em 1d8 para cada círculo de espaço de magia acima de 1.",
    descricaoCurta: "Raio esverdeado à distância. Ataque: 2d8 dano Venenoso + Envenenado até o fim do próximo turno do alvo. Upcast: +1d8.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "dado-por-circulo",
    upcastCirculoBase: 1,
    upcastDado: "1d8",
  ...
  },
  {
    ...
    escola: "Evocação",
    classes: ["Bruxo"],
    tempoConjuracao: "Reação, que você realiza ao receber dano de uma criatura à sua vista e a até 18 metros de você",
    alcance: "18 metros",
    componentes: "V, S",
    duracao: "Instantânea",
    descricaoCompleta: "A criatura que lhe causou dano é momentaneamente cercada por chamas verdes. Ela realiza uma salvaguarda de Destreza, sofrendo 2d10 pontos de dano Ígneo se falhar, ou metade desse dano em caso de sucesso. Usando um Espaço de Magia de Círculo Superior. O dano aumenta em 1d10 para cada círculo de espaço de magia acima de 1.",
    descricaoCurta: "Reação ao sofrer dano de alguém à vista (até 18m): chamas verdes o cercam. Salv. Destreza ou 2d10 dano Ígneo (sucesso=metade). Upcast: +1d10.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "dado-por-circulo",
    upcastCirculoBase: 1,
    upcastDado: "1d10",
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "Concentração, até 10 minutos",
    descricaoCompleta: "Você executa a ação Correr e, até que a magia termine, pode executar essa ação novamente como uma Ação Bônus. Um Clérigo humano conjura Reviver os Mortos para trazer um camarada caído de volta à vida.",
    descricaoCurta: "Usa Correr; pode repetir Correr como Ação Bônus enquanto durar.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    duracao: "1 minuto",
    descricaoCompleta: "Você toca uma criatura voluntária. Uma vez em cada um dos turnos dela até que a magia encerre, essa criatura pode saltar até 9 metros gastando 3 metros de movimento. Usando um Espaço de Magia de Círculo Superior. Você pode escolher uma criatura adicional para cada círculo de espaço de magia acima de 1.",
    descricaoCurta: "Toca; 1x por turno, a criatura pode saltar até 9m gastando só 3m de movimento, enquanto durar. Upcast: +1 alvo/círculo.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "alvo-por-circulo",
    upcastCirculoBase: 1,
    upcastDado: null,
  ...
  },
  {
    ...
    escola: "Invocação",
    classes: ["Guardião"],
    tempoConjuracao: "Ação Bônus, que você realiza imediatamente após atingir uma criatura com um ataque com arma à Distância",
    alcance: "Pessoal",
    componentes: "V",
    duracao: "Instantânea",
    descricaoCompleta: "Ao atingir uma criatura, essa magia faz surgir uma chuva de espinhos a partir da sua arma à distância ou munição. O alvo e cada criatura em um raio de 1,5 metro devem realizar uma salvaguarda de Destreza, sofrendo 1d10 pontos de dano perfurante se falharem ou metade desse dano em caso de sucesso. Usando um Espaço de Magia de Círculo Superior. O dano aumenta em 1d10 para cada círculo de espaço de magia acima de 1.",
    descricaoCurta: "Após acertar com arma à distância: espinhos explodem — alvo + tudo num raio de 1,5m: Salv. Destreza ou 1d10 dano Perfurante (sucesso=metade). Upcast: +1d10.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "dado-por-circulo",
    upcastCirculoBase: 1,
    upcastDado: "1d10",
  ...
  },
  {
    ...
    descricaoCurta: "Melodia dissonante na mente do alvo. Salv. Sabedoria ou 3d6 dano Psíquico + usa a Reação pra fugir de você pela rota mais segura (sucesso=metade do dano, sem fuga forçada). Upcast: +1d6.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "dado-por-circulo",
    upcastCirculoBase: 1,
    upcastDado: "1d6",
  ...
  },
  {
    ...
    tempoConjuracao: "Ação",
    alcance: "Pessoal",
    componentes: "V, S, M (uma gota de álcool)",
    duracao: "Instantânea",
    descricaoCompleta: "Você obtém 2d4 + 4 Pontos de Vida Temporários. Usando um Espaço de Magia de Círculo Superior. Você obtém 5 Pontos de Vida Temporários adicionais para cada círculo de espaço de magia acima de 1. Um gnomo Bardo conjura Zombaria Perversa para aborrecer um kobold com insultos mágicos.",
    descricaoCurta: "Ganha 2d4+4 PV temporários. Upcast: +5 PV temp por círculo.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "flat-por-circulo",
    upcastCirculoBase: 1,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Você molda seu próprio corpo. Escolha 1 (pode trocar com Ação de Usar Magia): Adaptação Aquática (guelras, respira debaixo d'água, Deslocamento de Natação = Deslocamento); Armas Naturais (garras/presas/chifres/cascos — Ataque Desarmado vira 1d6 do tipo, usa atributo de conjuração); Mudar Aparência (altura/peso/rosto/voz/espécie, sem mudar tamanho/estatística).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "Ação",
    alcance: "Toque",
    componentes: "V, S, M (pelo ou uma pena) +0",
    duracao: "Concentração, até 1 hora",
    descricaoCompleta: "–5 Você toca uma criatura e escolhe Força, Destreza, Inteligência, Sabedoria ou Carisma. Pela duração da magia, o alvo tem Vantagem nos testes de atributo usando o atributo escolhido. Usando um Espaço de Magia de Círculo Superior. Você pode escolher uma criatura adicional para cada círculo de espaço de magia acima de 2. Você pode escolher um atributo diferente para cada alvo.",
    descricaoCurta: "Toca; escolhe 1 atributo — Vantagem em testes desse atributo, pela duração (1h). Upcast: +1 alvo/círculo (atributo pode variar por alvo).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "alvo-por-circulo",
    upcastCirculoBase: 2,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Cria uma arma espectral flutuante à escolha. Ataque corpo a corpo imediato: 1d8 + mod. de conjuração dano Energético. Ação Bônus em turnos seguintes: move 6m + repete o ataque. Upcast: +1d8.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "dado-por-circulo",
    upcastCirculoBase: 2,
    upcastDado: "1d8",
  ...
  },
  {
    ...
    descricaoCurta: "Toca arma não-mágica: vira +1 no ataque/dano pela duração (1h). Encerra se reconjurar. Upcast: +2 com espaço 3º-5º círculo; +3 com 6º+.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "outro",
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V",
    duracao: "Instantânea",
    descricaoCompleta: "Escolha um objeto à sua vista e no alcance da magia. O objeto pode ser uma porta, uma caixa, um baú, um con- Um Golias Clérigo conjura Arma Espiritual em batalha contra um Tríbulo Brutal. junto de grilhões, um cadeado ou outro objeto que contenha um meio mundano ou mágico que impeça o acesso. Um alvo mantido fechado por uma fechadura mundana ou que está preso ou barrado fica destrancado, desemperrado ou desobstruído. Se o objeto tiver várias fechaduras, apenas uma delas é destrancada. Se o alvo for mantido fechado por Tranca Arcana, essa magia é suprimida por 10 minutos, durante os quais o alvo pode ser aberto e fechado. Ao conjurar a magia, um estrondo, audível a até 90 metros de distância, emana do alvo.",
    descricaoCurta: "Destranca/desemperra 1 fechadura mundana à vista (só 1 se houver várias); suprime Tranca Arcana por 10 min. Gera estrondo audível a 90m.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    duracao: "Instantânea",
    descricaoCompleta: "Você recebe um presságio de uma entidade sobrenatural a respeito dos resultados de um curso de ação que você planeja realizar nos próximos 30 minutos. O Mestre escolhe o presságio da tabela Presságios. Presságios Presságios                  Para Resultados Que Serão… Prosperidade                Bons Infortúnio                  Maus Prosperidade e Infortúnio   Bons e ruins Nada                        Nem bons, nem ruins A magia não leva em conta circunstâncias, como outras magias, que podem alterar os resultados. Se você conjurar a magia mais de uma vez antes de terminar um Descanso Longo, há uma chance cumulativa de 25% para cada conjuração após a primeira de que você não receba resposta.",
    descricaoCurta: "Pede um presságio sobre um curso de ação nos próximos 30 min: Prosperidade (bom), Infortúnio (mau), Ambos, ou Nada — mestre decide. Não considera fatores que podem mudar o resultado depois. Repetir antes de Descanso Longo: 25% cumulativo de não receber resposta a cada vez.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Escolhe Aumentar ou Reduzir 1 criatura/objeto à vista. Salv. Constituição anula se involuntária. Aumentar: +1 categoria de tamanho, Vantagem em testes/salv. de Força, +1d4 dano em ataques. Reduzir: -1 categoria, Desvantagem de Força, -1d4 dano (mín. 1). Equipamento acompanha a mudança.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    duracao: "8 horas",
    descricaoCompleta: "Escolha até três criaturas no alcance da magia. Os Pontos de Vida máximos e os Pontos de Vida atuais de cada alvo aumentam em 5 pela duração da magia. Usando um Espaço de Magia de Círculo Superior. Os Pontos de Vida de cada alvo aumentam em 5 para cada círculo de espaço de magia acima de 2.",
    descricaoCurta: "Até 3 criaturas: PV máximo e atual +5 pela duração (8h). Upcast: +5 PV por círculo.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "flat-por-circulo",
    upcastCirculoBase: 2,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Implanta mensagem (até 25 palavras) num objeto, revelada por um gatilho visual/sonoro à sua escolha num raio de 9m do objeto. Uma boca espectral fala com sua voz quando dispara; você escolhe se repete sempre ou só uma vez. Dura até ser dissipada.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "Ação",
    alcance: "Toque",
    componentes: "V, S, M (rubi em pó no valor de 50 ou mais PO, que a magia consome)",
    duracao: "Até ser dissipada",
    descricaoCompleta: "Uma chama brota de um objeto que você toca. O efeito conjura Luz Plena em um raio de 6 metros e Meia-luz por mais 6 metros. Ela se parece uma chama comum, mas não cria calor e não consome combustível. A chama pode ser coberta ou escondida, mas não abafada ou extinta.",
    descricaoCurta: "Toca objeto: chama fria (sem calor, não consome combustível) emite Luz Plena 6m + Meia-luz 6m, até ser dissipada. Pode ser coberta mas não apagada.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "Instantânea",
    descricaoCompleta: "Você convoca um ser sobrenatural que aparece como uma montaria leal em um espaço desocupado à sua escolha no alcance da magia. Essa criatura usa o bloco de estatísticas de Montaria Sobrenatural. Se você já tem uma montaria desta magia, a montaria é substituída pela nova. Montaria Sobrenatural Celestial Grande, Feérico ou Ínfero (À Sua Escolha), Neutro CA 10 + 1 por círculo de magia PV 5 + 10 por círculo de magia (a montaria tem um número de Dados de Vida [d10s] igual ao círculo da magia) Deslocamento 18 m, Voo 18 m (requer magia de 4º círculo ou superior) For 18     +4   +4     Des 12    +1    +1    Con 14     +2 Int 6      –2   –2     Sab 12    +1    +1    Car 8      –1 Sentidos Percepção Passiva 11 Idiomas Telepatia 1,5 km (funciona apenas com você) ND Nenhum (XP 0; BP é igual ao seu Bônus de Proficiência) Traços Vínculo Vital. Ao recuperar Pontos de Vida de uma magia 1º círculo ou superior, a montaria recupera o mesmo número de Pontos de Vida se você estiver a até 1,5 metro dela. Ações Pancada Sobrenatural. Jogada de Ataque Corpo a Corpo: Bônus é igual ao seu modificador de ataque mágico, alcance 1,5 m. Dano: 1d8 mais o círculo da magia de pontos de dano Radiante (Celestial), Psíquico (Feérico) ou Necrótico (Ínfero). Ações Bônus Derrubar Brilho (Apenas Ínfero; Recarrega após um Descanso Longo). Salvaguarda de Sabedoria: CD é igual a CD para evitar sua magia, uma criatura a até 18 metros à vista da montaria. Falha: O alvo tem a condição Amedrontado até o final do seu próximo turno. Passo Feérico (Apenas Feérico; Recarrega após um Descanso Longo). A montaria se teleporta, junto com seu cavaleiro, para um espaço desocupado à sua escolha a até 18 metros de distância de onde está. Toque Curativo (Apenas Celestial; Recarrega após um Descanso Longo). Uma criatura a até 1,5 metro da montaria recupera um número de Pontos de Vida igual a 2d8 mais o círculo da magia. A montaria se assemelha a um animal grande e que se pode montar à sua escolha, como um alce, um camelo, um cavalo ou um lobo atroz. Sempre que conjurar a magia, escolha o tipo de criatura da montaria — Celestial, Feérico ou Ínfero — que determina certas características no bloco de estatísticas. Combate. A montaria é um aliado seu e de seus aliados. Em combate, ela compartilha sua contagem de Iniciativa e funciona como uma montaria controlada enquanto você a monta (conforme definido nas regras sobre combate montado). Se você tem a condição Incapacitado, a montaria age no turno dela imediatamente após o seu, de forma independente, concentrando-se em protegê-lo. Desaparecimento da Montaria. A montaria desaparece se é reduzida a 0 Pontos de Vida ou se você morrer. Quando desaparece, deixa para trás qualquer coisa que estivesse vestindo ou carregando. Se você conjurar esta magia novamente, você decide se invoca a montaria que desapareceu ou uma diferente. Usando um Espaço de Magia de Círculo Superior. Use o círculo do espaço de magia para o círculo da magia no bloco de estatísticas.",
    descricaoCurta: "Invoca uma montaria sobrenatural (aparência de animal grande à escolha; tipo Celestial/Feérico/Ínfero). Compartilha sua Iniciativa; some a 0 PV ou se você morrer. Upcast: círculo do espaço = círculo da montaria (mais forte).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "formula-propria",
    upcastCirculoBase: null,
    upcastDado: null,
    upcastFlat: null,
    upcastAlvos: null,
  ...
  },
  {
    ...
    duracao: "Instantânea",
    descricaoCompleta: "Um barulho alto irrompe de um ponto à sua escolha no alcance da magia. Cada criatura em uma esfera com raio de 3 metros centrada nesse ponto realiza uma salvaguarda de Constituição, sofrendo 3d8 pontos de dano Trovejante se falhar, ou metade desse dano em caso de sucesso. Um Constructo tem Desvantagem na salvaguarda. Um objeto não mágico que não esteja sendo usado ou carregado também sofre o dano se estiver na área da magia. Usando um Espaço de Magia de Círculo Superior. O dano aumenta em 1d8 para cada círculo de espaço de magia acima de 2.",
    descricaoCurta: "Estrondo numa esfera de 3m. Salv. Constituição ou 3d8 dano Trovejante (sucesso=metade); Constructos têm Desvantagem. Afeta objetos soltos também. Upcast: +1d8.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "dado-por-circulo",
    upcastCirculoBase: 2,
    upcastDado: "1d8",
  ...
  },
  {
    ...
    descricaoCurta: "Ação Bônus após acertar corpo a corpo/desarmado: +2d6 dano Radiante; até acabar (1 min), alvo emite Luz Plena 1,5m, sofre ataques com Vantagem e perde benefício de Invisível. Upcast: +1d6.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "dado-por-circulo",
    upcastCirculoBase: 2,
    upcastDado: "1d6",
  ...
  },
  {
    ...
    descricaoCurta: "Toca; ganha Deslocamento de Escalada = Deslocamento normal, sobe paredes/tetos com mãos livres, pela duração (1h). Upcast: +1 alvo/círculo.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "alvo-por-circulo",
    upcastCirculoBase: 2,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "Ação",
    alcance: "18 metros",
    componentes: "V, M (pelo de morcego e um pedaço de carvão)",
    duracao: "Concentração, até 10 minutos",
    ...
    descricaoCurta: "Escuridão mágica preenche esfera de 4,5m (ou Emanação de 4,5m num objeto, bloqueável cobrindo o objeto). Visão no Escuro não penetra; luz não-mágica não ilumina. Dissipa magias de Luz de 2º círculo ou menor sobrepostas.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    duracao: "Concentração, até 1 hora",
    descricaoCompleta: "+2 Uma criatura que você toca tem a condição Invisível +2 até que a magia termine. A magia se encerra antes se o alvo realizar uma jogada de ataque, causar dano ou conjurar uma magia. Usando um Espaço de Magia de Círculo Superior. Você pode escolher uma criatura adicional para cada círculo de espaço de magia acima de 2.",
    descricaoCurta: "Toca: Invisível pela duração (1h). Encerra se atacar, causar dano ou conjurar. Upcast: +1 alvo/círculo.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "alvo-por-circulo",
    upcastCirculoBase: 2,
    upcastDado: null,
  ...
  },
  {
    ...
    duracao: "Instantânea",
    descricaoCompleta: "Descreva ou nomeie uma criatura do tipo Fera, Planta ou planta não mágica. Você descobre a direção e a distância até a criatura ou planta mais próxima desse tipo a até 8 quilômetros, se houver alguma presente.",
    descricaoCurta: "Descreve/nomeia 1 Fera ou Planta; descobre direção e distância até a mais próxima desse tipo, em até 8km.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V",
    duracao: "Instantânea",
    descricaoCompleta: "Até cinco criaturas à sua escolha que permanecerem no alcance da magia por toda a conjuração obtêm os benefícios de um Descanso Curto e recuperam 2d8 Pontos de Vida. Uma criatura não pode ser afetada por esta magia novamente até ela completar um Descanso Longo. Usando um Espaço de Magia de Círculo Superior. A cura aumenta em 1d8 para cada círculo de espaço de magia acima de 2.",
    descricaoCurta: "Até 5 criaturas no alcance por toda a conjuração (10 min) ganham benefício de Descanso Curto + curam 2d8 PV. Não pode repetir na mesma criatura até Descanso Longo. Upcast: +1d8.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "dado-por-circulo",
    upcastCirculoBase: 2,
    upcastDado: "1d8",
  ...
  },
  {
    ...
    componentes: "V",
    duracao: "Instantânea",
    descricaoCompleta: "Brevemente cercado por uma névoa prateada, você se teleporta até 9 metros para um espaço desocupado à sua vista.",
    descricaoCurta: "Ação Bônus: teleporta até 9m pra espaço desocupado à vista, envolto em névoa prateada.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    duracao: "Concentração, até 1 hora",
    descricaoCompleta: "Você irradia uma aura de ocultação em uma Emanação de 9 metros pela duração da magia. Enquanto estiver na aura, você e cada criatura escolhida têm um bônus de +10 em testes de Destreza (Furtividade) e não deixam rastros.",
    descricaoCurta: "Aura de 9m ao redor: você e quem escolher têm +10 em Furtividade e não deixam rastro, pela duração (1h).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "Ação Bônus",
    alcance: "Toque",
    componentes: "V, S, M (um punhado de casca de árvore)",
    duracao: "1 hora",
    descricaoCompleta: "Você toca uma criatura voluntária. Até que a magia termine, a pele do alvo assume uma aparência de casca de árvore, e o alvo tem uma Classe de Armadura de 17 se sua CA for menor que isso.",
    descricaoCurta: "Toca: pele vira casca de árvore — CA mínima 17 pela duração (1h) se a CA normal for menor.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "1 hora",
    descricaoCompleta: "Você toca uma criatura e encerra nela a condição Envenenado. Pela duração da magia, o alvo tem Vantagem nas salvaguardas para evitar ou encerrar a condição Envenenado e tem Resiste a dano Venenoso.",
    descricaoCurta: "Toca: remove Envenenado; Vantagem em salvaguardas contra/pra encerrar Envenenado + Resistência a dano Venenoso, por 1h.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "Instantânea",
    descricaoCompleta: "Você dispara três raios flamejantes. Você pode dispará-los em um ou vários alvos no alcance da magia. Realize um ataque mágico à distância para cada raio. Em caso de acerto, o alvo sofre 2d6 pontos de dano Ígneo. Usando um Espaço de Magia de Círculo Superior. Você cria um raio adicional para cada círculo de espaço de magia acima de 2.",
    descricaoCurta: "3 raios de fogo, atribuíveis a 1+ alvos. Ataque à distância cada: 2d6 Ígneo. Upcast: +1 raio/círculo.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "alvo-por-circulo",
    upcastCirculoBase: 2,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "Ação ou Ritual",
    alcance: "Toque",
    componentes: "V, S, M (2 Peças de Cobre, que a magia consome)",
    duracao: "10 dias",
    descricaoCompleta: "Você toca um cadáver ou outros restos mortais. Pela duração da magia, o alvo é protegido da decomposição e não pode se tornar um Morto-Vivo. A magia também estende efetivamente o limite de tempo para ressuscitar o alvo dos mortos, uma vez que os dias passados sob a influência desta magia não contam contra o limite de tempo de magias como Reviver os Mortos.",
    descricaoCurta: "Toca cadáver: protege da decomposição e de virar Morto-vivo por 10 dias; esses dias não contam pro prazo de Reviver os Mortos.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "Instantânea",
    descricaoCompleta: "Você toca uma criatura e remove uma condição: Cego, Envenenado, Paralisado ou Surdo.",
    descricaoCurta: "Toca: remove 1 condição — Cego, Envenenado, Paralisado ou Surdo.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "S",
    duracao: "Concentração, até 1 hora",
    descricaoCompleta: "Você toca uma Fera voluntária. Pela duração da magia, você pode perceber através dos sentidos da Fera, bem como dos seus próprios. Ao perceber através dos sentidos da Fera, você se beneficia de quaisquer sentidos especiais que ela tenha.",
    descricaoCurta: "Toca Fera voluntária: percebe pelos sentidos dela (inclusive especiais) além dos seus, pela duração (1h).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Esfera de 6m onde nenhum som é criado/passa; imunidade a dano Trovejante e Surdo dentro; impossível conjurar magia com componente Verbal lá dentro.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    duracao: "Concentração, até 1 minuto",
    descricaoCompleta: "Você toca uma criatura voluntária e escolhe Ácido, Elétrico, Gélido, Ígneo ou Venenoso. Até que a magia termine, o alvo pode executar uma ação Usar Magia para exalar um Cone de 4,5 metros. Cada criatura nessa área realiza uma salvaguarda de Destreza, sofrendo 3d6 pontos de dano do tipo escolhido se falhar, ou metade desse dano em caso de sucesso. Usando um Espaço de Magia de Círculo Superior. O dano aumenta em 1d6 para cada círculo de espaço de magia acima de 2.",
    descricaoCurta: "Toca; escolhe tipo de dano (Ácido/Elétrico/Gélido/Ígneo/Venenoso). Alvo pode (Ação de Usar Magia) exalar Cone de 4,5m: Salv. Destreza ou 3d6 do tipo (sucesso=metade). Upcast: +1d6.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "dado-por-circulo",
    upcastCirculoBase: 2,
    upcastDado: "1d6",
  ...
  },
  {
    ...
    tempoConjuracao: "Ação",
    alcance: "Toque",
    componentes: "V, S, M (pó de ouro no valor de 25 ou mais PO, que a magia consome)",
    duracao: "Até ser dissipada",
    descricaoCompleta: "Você toca uma porta, janela, portão, contêiner ou escotilha fechada e a tranca magicamente pela duração da magia. Esta fechadura não pode ser destrancada por nenhum meio não mágico. Você e quaisquer criaturas que você designar ao conjurar a magia podem abrir e fechar o objeto apesar da tranca. Você também pode definir uma senha que, quando proferida a até 1,5 metro do objeto, o destranca por 1 minuto.",
    descricaoCurta: "Toca porta/janela/contêiner: tranca magicamente (só destranca por magia), até ser dissipada. Você/quem designar pode abrir normalmente; pode definir senha (destranca por 1 min ao ser dita perto).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V",
    duracao: "Concentração, até 1 minuto",
    descricaoCompleta: "Seu corpo fica desfocado. Pela duração da magia, qualquer criatura tem Desvantagem em jogadas de ataque contra você. Um atacante é imune a esse efeito se perceber você com Visão às Cegas ou Visão Verdadeira.",
    descricaoCurta: "Seu corpo fica desfocado — Desvantagem em ataques contra você pela duração (1 min), exceto quem tem Visão às Cegas/Verdadeira.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "Ação",
    alcance: "Pessoal",
    componentes: "V, S, M (uma pitada de talco)",
    duracao: "1 hora",
    descricaoCompleta: "Pela duração da magia, você vê criaturas e objetos que têm a condição Invisível como se estivessem visíveis e pode ver o Plano Etéreo. Criaturas e objetos nele tem aparência fantasmagórica. A magia Vidência ajuda um gnomo Bardo a manter contato com amigos e familiares em todo o mundo.",
    descricaoCurta: "Vê criaturas/objetos Invisíveis e enxerga o Plano Etéreo (fantasmagórico), pela duração (1h).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "Instantânea",
    descricaoCompleta: "Você usa sua energia vital para se curar. Jogue um ou dois de seus Dados de Pontos de Vida não gastos e recupera um número de Pontos de Vida igual ao total do teste mais seu modificador de atributo de conjuração. Esses dados são, então, gastos. Usando um Espaço de Magia de Círculo Superior. O número de Dados de Vida não gastos que você pode jogar aumenta em um para cada círculo de espaço de magia acima de 2.",
    descricaoCurta: "Ação Bônus: rola 1-2 dos seus Dados de Vida não gastos, cura o total + mod. de conjuração; os dados são gastos. Upcast: +1 dado/círculo.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "flat-por-circulo",
    upcastCirculoBase: 2,
    upcastDado: null,
  ...
  },
  {
    ...
    duracao: "8 horas",
    descricaoCompleta: "Pela duração da magia, uma criatura voluntária que você tocar tem Visão no Escuro com um alcance de 45 metros.",
    descricaoCurta: "Toca: Visão no Escuro 45m pela duração (8h).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V",
    duracao: "Concentração, até 1 minuto",
    descricaoCompleta: "Uma aura irradia de você em uma Emanação de 9 metros pela duração da magia. Quando você cria a aura e no início de cada um dos seus turnos enquanto ela persiste, você pode restaurar 2d6 Pontos de Vida em uma criatura dentro dela.",
    descricaoCurta: "Aura de 9m ao seu redor; ao criar e no início de cada seu turno, cura 2d6 PV em 1 criatura dentro dela.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    duracao: "Instantânea",
    descricaoCompleta: "Uma faixa brilhante emerge de você até um ponto à sua escolha no alcance da magia e, em seguida, desabrocha com um estrondo baixo em uma explosão de fogo. Cada criatura em uma Esfera de 6 metros de raio centrada nesse ponto realiza uma salvaguarda de Destreza, sofrendo 8d6 pontos de dano Ígneo se falhar, ou metade desse dano em caso de sucesso. Objetos inflamáveis na área que não estão sendo usados ou carregados entram em combustão. Usando um Espaço de Magia de Círculo Superior. O dano aumenta em 1d6 para cada círculo de espaço de magia acima de 3.",
    descricaoCurta: "Explosão numa esfera de 6m. Salv. Destreza ou 8d6 Ígneo (sucesso=metade); incendeia inflamáveis desprotegidos. Upcast: +1d6.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "dado-por-circulo",
    upcastCirculoBase: 3,
    upcastDado: "1d6",
  ...
  },
  {
    ...
    tempoConjuracao: "10 minutos",
    alcance: "1,5 km",
    componentes: "V, S, M (um foco no valor de 100 ou mais PO, seja um chifre adornado com joias e usado para escutar, ou um olho de vidro usado para enxergar)",
    duracao: "Concentração, até 10 minutos",
    descricaoCompleta: "Você cria um sensor Invisível no alcance da magia em um local que lhe seja familiar (um lugar que você já viu ou visitou antes) ou em um local óbvio que não lhe seja familiar (como atrás de uma porta, em uma esquina ou em um bosque). O sensor não pode ser atacado e permanece no lugar pela duração da magia. Ao conjurar a magia, escolha ver ou ouvir. Você pode usar o sentido escolhido através do sensor como se estivesse em seu espaço. Como uma Ação Bônus, você pode alternar entre ver e ouvir. Uma criatura que veja o sensor (como uma criatura que se beneficia de Ver o Invisível ou Visão Verdadeira) enxerga uma esfera luminosa do tamanho do seu punho.",
    descricaoCurta: "Cria sensor invisível e imóvel num local conhecido (ou óbvio, tipo atrás de porta). Escolhe ver OU ouvir por ele (troca por Ação Bônus), pela duração (10 min). Quem vê Invisível/Visão Verdadeira enxerga o sensor como esfera luminosa.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    escola: "Abjuração",
    classes: ["Bruxo", "Feiticeiro", "Mago"],
    tempoConjuracao: "Reação, que você executa quando vê uma criatura a até 18 metros de você conjurando uma magia com componentes Verbais, Somáticos ou Materiais",
    alcance: "18 metros",
    componentes: "S",
    duracao: "Instantânea",
    descricaoCompleta: "Você tenta interromper uma criatura no processo de conjurar uma magia. A criatura realiza uma salvaguarda de Constituição. Se falhar, a magia se dissipa sem efeito, e a ação, Ação Bônus ou Reação usada para conjurá-la é desperdiçada. Se a magia do alvo foi conjurada com um espaço de magia, o espaço de magia da criatura não é gasto.",
    descricaoCurta: "Reação ao ver alguém a até 18m conjurando (com componente V/S/M): Salv. Constituição do conjurador ou a magia dele se dissipa sem efeito (ação/espaço desperdiçados).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "Ação",
    alcance: "27 metros",
    componentes: "V, S, M (uma flor dourada no valor de 300 ou mais PO)",
    duracao: "Concentração, até 1 hora",
    descricaoCompleta: "Você invoca um Espírito Feérico. Ele se manifesta em um espaço desocupado à sua vista e no alcance da magia e usa o bloco de estatísticas do Espírito Feérico. Ao conjurar a magia, escolha um humor: Alegre, Enfurecido ou Malandro. A criatura se assemelha a uma criatura Feérica à sua escolha definida pelo humor escolhido, o que determina certos detalhes no bloco de estatísticas da criatura. A criatura desaparece se reduzida a 0 Pontos de Vida ou quando a magia termina. A criatura é uma aliada sua e de seus aliados. Em combate, ela compartilha sua contagem de Iniciativa, mas o turno dela é imediatamente após o seu. Ela obedece aos seus comandos verbais (nenhuma ação é ne- Um Guardião conjura Convocar Feérico, conjurando um Espírito Feérico enfurecido para enfrentar um Merrow saqueador. Espírito Feérico Feérico Pequeno, Neutro CA 12 + o círculo da magia PV 30 + 10 para cada círculo de magia acima de 3 Deslocamento 9 m, Voo 9 m For 13     +1   +1    Des 16     +3   +3    Con 14     +2   +2 Int 14     +2   +2    Sab 11     +0   +0    Car 16     +3   +3 Imunidades Enfeitiçado Sentidos Visão no Escuro 18 m, Percepção Passiva 10 Idiomas Silvestre, compreende os idiomas que você fala ND Nenhum (XP 0; BP é igual ao seu Bônus de Proficiência) Ações Ataques Múltiplos. O espírito realiza um número de ataques com Lâmina Feérica igual à metade do círculo da magia (arredondado para baixo). Lâmina Feérica. Jogada de Ataque Corpo a Corpo: Bônus é igual ao seu modificador de ataque mágico, alcance 1,5 m. Dano: 2d6 + 3 + o círculo da magia pontos de dano Energético. Ações Bônus Passo Feérico. O espírito se teleporta magicamente até 9 metros para um espaço desocupado à vista dele. Em seguida, ocorre um dos seguintes efeitos, com base no humor escolhido para o espírito: Alegre. Salvaguarda de Sabedoria: CD é igual a CD para evitar sua magia, uma criatura à vista do espírito e a até 3 metros dele. Falha: O alvo fica Enfeitiçado por você e pelo espírito por 1 minuto ou até que o alvo sofra qualquer dano. Enfurecido. O espírito tem Vantagem na próxima jogada de ataque que realizar antes do final deste turno. Malandro. O espírito preenche um Cubo de 3 metros de lado a até 1,5 metro dele com Escuridão mágica, que dura até o final do próximo turno dele. cessária). Se você não emitir nenhum, ela executa a ação Esquivar e usa o movimento dela para evitar o perigo. Usando um Espaço de Magia de Círculo Superior. Use o círculo do espaço de magia para o círculo da magia no bloco de estatísticas.",
    descricaoCurta: "Invoca um Espírito Feérico (humor Alegre/Enfurecido/Malandro à escolha, define aparência e habilidade de Ação Bônus: Alegre=tenta Enfeitiçar perto; Enfurecido=Vantagem no próximo ataque; Malandro=Escuridão num cubo 3m). Aliado, compartilha Iniciativa, ataca com Lâmina Feérica (2d6+3+círculo Energético). Upcast: círculo do espaço = força.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "formula-propria",
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Cria 20kg de comida + 120L de água potável no alcance. Comida não temperada mas nutritiva; estraga em 24h.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Ação Bônus após acertar corpo a corpo/desarmado: +3d8 dano Radiante + Cego até acabar (Salv. Constituição no fim de cada turno dele pra encerrar, 1 min). Upcast: +1d8.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "dado-por-circulo",
    upcastCirculoBase: 3,
    upcastDado: "1d8",
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "Instantânea",
    descricaoCompleta: "Escolha uma criatura, objeto ou efeito mágico no alcance da magia. Qualquer magia ativa de 3º círculo ou inferior no alvo é encerrada. Para cada magia ativa de 4º círculo ou superior no alvo, realize um teste de atributo usando seu atributo de conjuração (CD 10 mais o círculo da magia alvo). Se falhar, a magia continua; se for bem-sucedido, a magia se encerra. Usando um Espaço de Magia de Círculo Superior. Você encerra automaticamente uma magia no alvo se o círculo da magia for igual ou inferior ao círculo do espaço de magia que você usa. Um tiferino Clérigo conjura Dissipar o Bem e o Mal para expulsar um espírito maligno de um inocente possuído.",
    descricaoCurta: "Encerra automaticamente qualquer magia ativa de 3º círculo ou menor no alvo. Pra cada uma de 4º+: teste do seu atributo de conjuração (CD 10+círculo da magia alvo) — sucesso encerra. Upcast: encerra automático até o círculo do espaço usado.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "formula-propria",
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Ilusão completa (som/cheiro/temperatura) até cubo de 6m, movível por Ação de Usar Magia com narração de movimento natural. Interação física ou Investigação vs sua CD revela. Dura 10 min de Concentração; upcast com espaço 4º+: dura até dissipar, sem Concentração.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "outro",
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "Ação",
    alcance: "Toque",
    componentes: "V, S, M (uma pitada de poeira de diamante no valor de 25 ou mais PO, que a magia consome)",
    duracao: "8 horas",
    descricaoCompleta: "Pela duração da magia, você protege um alvo que tocar contra magias de Adivinhação. O alvo pode ser uma criatura voluntária, um local ou um objeto que não exceda 3 metros em qualquer dimensão. O alvo não pode ser afetado por nenhuma magia de Adivinhação nem percebido através de sensores mágicos de vidência. Imagem Silenciosa pode criar ilusões mundanas e excêntricas.",
    descricaoCurta: "Toca (criatura/local/objeto até 3m): protegido de magias e sensores de Adivinhação, pela duração (8h).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "Ação",
    alcance: "27 metros",
    componentes: "V, S, M (um crânio dourado no valor de 300 ou mais PO)",
    duracao: "Concentração, até 1 hora",
    descricaoCompleta: "Você invoca um Espírito Morto-Vivo. Ele se manifesta em um espaço desocupado à sua vista e no alcance da magia e usa o bloco de estatísticas do Espírito Morto- -vivo. Ao conjurar a magia, escolha a forma da criatura: Esquelético, Fantasmagórico ou Pútrido. O espírito se assemelha a uma criatura Morta-viva com a forma escolhida, o que determina certos detalhes no bloco de estatísticas da criatura. A criatura desaparece ao ser reduzida a 0 Pontos de Vida ou quando a magia termina. A criatura é uma aliada sua e de seus aliados. Em combate, ela compartilha sua contagem de Iniciativa, mas o turno dela é imediatamente após o seu. Ele obedece aos seus comandos verbais (nenhuma ação é necessária). Se você não emitir nenhum, ela executa a ação Esquivar e usa o movimento dela para evitar o perigo. Usando um Espaço de Magia de Círculo Superior. Use o círculo do espaço de magia para o círculo da magia no bloco de estatísticas. Espírito Morto-Vivo Morto-vivo Médio, Neutro CA 11 + o círculo da magia PV 30 (apenas Fantasmagórico e Pútrido) ou 20 (apenas Esquelético) + 10 para cada círculo de magia acima de 3 Deslocamento 9 m; Voo 12 m (pairar; apenas Fantasmagórico) For 12     +1   +1     Des 16    +3   +3     Con 15    +2    +2 Int 4      –3   –3     Sab 10    +0   +0     Car 9     –1    –1 Imunidades Necrótico, Venenoso; Amedrontado, Envenenado, Exaustão, Paralisado Sentidos Visão no Escuro 18 m, Percepção Passiva 10 Idiomas Compreende os idiomas que você fala ND Nenhum (XP 0; BP é igual ao seu Bônus de Proficiência) Traços Aura Purulenta (Apenas Pútrido). Salvaguarda de Constituição: CD é igual a CD para evitar sua magia, qualquer criatura (exceto você) que inicie o turno dentro de uma Emanação de 1,5 metro originada a partir do espírito. Falha: A criatura tem a condição Envenenado até o início de seu próximo turno. Passagem Incorpórea (Apenas Fantasmagórico). O espírito pode se mover através de outras criaturas e objetos como se fossem Terreno Difícil. Se ele terminar o turno dentro de Espírito Morto-vivo (Fantasmagórico) um objeto, ele é desviado para o espaço desocupado mais próximo e sofre 1d10 pontos de dano Energético por cada 1,5 metro percorrido. Ações Ataques Múltiplos. O espírito realiza um número de ataques igual à metade do círculo da magia (arredondado para baixo). Garra Podre (Apenas Pútrido). Jogada de Ataque Corpo a Corpo: Bônus é igual ao seu modificador de ataque mágico, alcance 1,5 m. Dano: 1d6 + 3 mais o círculo da magia em pontos de dano Cortante. Se o alvo tem a condição Envenenado, ele tem a condição Paralisado até o final do próximo turno dele. Raio da Cova (Somente Esquelético). Jogada de Ataque à Distância: Bônus é igual ao seu modificador de ataque mágico, alcance 45 m. Dano: 2d4 + 3 mais o círculo da magia em pontos de dano Necrótico. Toque Mortal (Apenas Fantasmagórico). Jogada de Ataque Corpo a Corpo: Bônus é igual ao seu modificador de ataque mágico, alcance 1,5 m. Dano: 1d8 + 3 mais o círculo da magia em pontos de dano Necrótico, e o alvo tem a condição Amedrontado até o final do próximo turno dele.",
    descricaoCurta: "Invoca Espírito Morto-vivo (forma Esquelético/Fantasmagórico/Pútrido à escolha, cada um com habilidade própria — veneno, incorporeidade, medo). Aliado, ataca por comando, some a 0 PV/fim. Upcast: círculo do espaço = força.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "formula-propria",
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    duracao: "Concentração, até 1 minuto",
    descricaoCompleta: "Você altera a passagem do tempo em torno de até seis criaturas à sua escolha em um Cubo de 12 metros de lado no alcance da magia. Cada alvo deve ser bem-sucedido em uma salvaguarda de Sabedoria ou é afetado por esta magia pela sua duração. O Deslocamento de um alvo afetado é reduzido pela metade, sofre uma penalidade de −2 em salvaguardas de Destreza e na CA e não pode executar Reações. Nos turnos do alvo, ele pode executar uma ação ou uma Ação Bônus, não ambas, e pode realizar apenas um ataque se executar a ação Atacar. Se conjurar uma magia com um componente Somático, há uma chance de 25% da magia falhar como resultado do alvo fazer os gestos da magia muito lentamente. Um alvo afetado repete a salvaguarda no final de cada um dos turnos dele, encerrando a magia em caso de sucesso.",
    descricaoCurta: "Até 6 criaturas num cubo de 12m: Salv. Sabedoria ou (pela duração, 1 min): Deslocamento -50%, -2 CA/salv. Destreza, sem Reação, só ação OU Ação Bônus (não ambas) e só 1 ataque se Atacar; 25% de falhar magia com componente Somático. Repete salvaguarda no fim de cada turno.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Luz do sol preenche esfera de 18m (Luz Plena) + Meia-luz por mais 18m, pela duração (1h). Dissipa Escuridão de 3º círculo ou menor sobreposta.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    duracao: "1 hora",
    descricaoCompleta: "Esta magia concede à criatura que você toca a capacidade de entender qualquer idioma falado ou com língua de sinais que ela ouça ou veja. Além disso, quando o alvo se comunica falando ou usando sinais, qualquer criatura que conheça pelo menos um idioma pode entendê-lo se essa criatura puder ouvir as palavras ou ver os sinais.",
    descricaoCurta: "Toca: entende qualquer idioma falado/em sinais que ouça/veja; quando ele fala/sinaliza, qualquer um que saiba 1 idioma o entende, pela duração (1h).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V",
    duracao: "Concentração, até 1 minuto",
    descricaoCompleta: "Você irradia uma aura mágica em uma Emanação de 9 metros. Enquanto estiver na aura, você e seus aliados causam 1d4 pontos de dano Radiante adicionais ao atingir com uma arma ou um Ataque Desarmado.",
    descricaoCurta: "Emanação de 9m: você e aliados dentro causam +1d4 Radiante em acertos com arma/desarmado, pela duração (1 min).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    duracao: "Concentração, até 1 minuto",
    descricaoCompleta: "Cada criatura em um Cone de 9 metros deve ser bem- -sucedida em uma salvaguarda de Sabedoria ou larga o que estiver segurando e tem a condição Amedrontado pela duração da magia. Uma criatura Amedrontada executa a ação Correr e se afasta de você pela rota mais segura em cada um dos turnos dela, a menos que não haja para onde se mover. Se a criatura encerrar o turno dela em um espaço onde não tenha linha de visão para você, a criatura realiza uma salvaguarda de Sabedoria. Em caso de sucesso, a magia encerra naquela criatura.",
    descricaoCurta: "Cone de 9m: Salv. Sabedoria ou larga o que segura e fica Amedrontado (corre pela rota mais segura todo turno), pela duração (1 min). Fora de linha de visão no fim do turno: repete a salvaguarda, sucesso encerra.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Granizo num cilindro 12m alt x6m raio: Totalmente Obscurecido, apaga chamas expostas, Terreno Difícil. 1ª vez que entra/começa turno: Salv. Destreza ou Caído + perde Concentração.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "Ação",
    alcance: "27 metros",
    componentes: "V, S, M (um ovo podre)",
    duracao: "Concentração, até 1 minuto",
    ...
    descricaoCurta: "Esfera de 6m de gás amarelo, Totalmente Obscurecida, dispersa por vento forte. Quem começa turno lá: Salv. Constituição ou Envenenado até o fim do turno (sem ação/Ação Bônus).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "Ação",
    alcance: "36 metros",
    componentes: "S, M (uma pitada de confete)",
    duracao: "Concentração, até 1 minuto",
    descricaoCompleta: "Você cria um padrão distorcido de cores em um cubo de 9 metros de lado no alcance da magia. O padrão aparece por um momento e desaparece. Cada criatura na área, que pode ver o padrão, deve ser bem-sucedida em uma salvaguarda de Sabedoria ou tem a condição Enfeitiçado pela duração. Enquanto Enfeitiçado, a criatura tem a condição Incapacitado e Deslocamento 0. A magia se encerra para uma criatura afetada se ela sofrer algum dano ou se outra pessoa executar uma ação para sacudi-la, removendo-a de seu estupor.",
    descricaoCurta: "Cubo de 9m com padrão de cores. Quem vê: Salv. Sabedoria ou Enfeitiçado + Incapacitado + Deslocamento 0, pela duração (1 min). Encerra no alvo se sofrer dano ou for sacudido.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V",
    duracao: "Instantânea",
    descricaoCompleta: "Até seis criaturas à sua escolha, à sua vista e no alcance da magia, recuperam Pontos de Vida iguais a 2d4 mais o seu modificador de atributo de conjuração. Usando um Espaço de Magia de Círculo Superior. A cura aumenta em 1d4 para cada círculo de espaço de magia acima de 3.",
    descricaoCurta: "Ação Bônus à distância: até 6 criaturas curam 2d4 + mod. conjuração PV. Upcast: +1d4.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "dado-por-circulo",
    upcastCirculoBase: 3,
    upcastDado: "1d4",
  ...
  },
  {
    ...
    circulo: 3,
    escola: "Abjuração",
    classes: ["Clérigo", "Druida", "Feiticeiro", "Guardião", "Mago"],
    tempoConjuracao: "Ação",
    alcance: "Toque",
    componentes: "V, S",
    duracao: "Concentração, até 1 hora",
    descricaoCompleta: "Pela duração da magia, a criatura voluntária que você tocar tem Resistência a um tipo de dano à sua escolha: Ácido, Elétrico, Gélido, Ígneo ou Trovejante.",
    descricaoCurta: "Toca: Resistência a 1 tipo (Ácido/Elétrico/Gélido/Ígneo/Trovejante) à escolha, pela duração (1h).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "Instantânea",
    descricaoCompleta: "Ao seu toque, todas as maldições que afetam uma criatura ou objeto se encerram. Se o objeto for um item mágico amaldiçoado, a maldição permanece, mas a magia quebra a Sintonização do dono com o objeto, permitindo que ele seja removido ou descartado.",
    descricaoCurta: "Toca: encerra todas as maldições na criatura/objeto. Item mágico amaldiçoado: maldição fica, mas quebra a Sintonização (pode ser removido/descartado).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "Ação ou Ritual",
    alcance: "9 metros",
    componentes: "V, S, M (um pedaço de junco)",
    duracao: "24 horas",
    descricaoCompleta: "Esta magia concede a até dez criaturas voluntárias à sua escolha no alcance da magia a capacidade de respirar debaixo d’água até que a magia termine. As criaturas afetadas também mantêm seu modo normal de respiração.",
    descricaoCurta: "Até 10 voluntários respiram debaixo d'água (mantendo respiração normal também), por 24h.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "Ação",
    alcance: "Toque",
    componentes: "V, S, M (um diamante no valor de 300 ou mais PO, que a magia consome)",
    duracao: "Instantânea",
    descricaoCompleta: "Você toca uma criatura que morreu no último minuto. Essa criatura revive com 1 Ponto de Vida. Esta magia não pode reviver uma criatura que morreu de velhice, nem restaura partes do corpo ausentes.",
    descricaoCurta: "Toca criatura morta há até 1 min: revive com 1 PV. Não funciona em morte por velhice nem restaura partes do corpo.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    duracao: "1 hora",
    descricaoCompleta: "Você toca uma criatura voluntária e a coloca em um estado cataléptico que é indistinguível da morte. Pela duração da magia, o alvo parece morto em uma inspeção externa e para magias usadas para determinar a situação do alvo. O alvo está com as condições Cego e Incapacitado, e o Deslocamento dele é 0. O alvo também tem Resistência a todos os tipos de dano, exceto dano Psíquico, e tem Imunidade à condição Envenenado.",
    descricaoCurta: "Toca: alvo parece morto pra inspeção externa e magias de detecção. Cego + Incapacitado + Deslocamento 0, Resistência a todo dano exceto Psíquico, imune a Envenenado, por 1h.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "Concentração, até 1 minuto",
    descricaoCompleta: "Escolha qualquer número de criaturas no alcance da magia. Pela duração da magia, cada alvo tem Vantagem em salvaguardas de Sabedoria e Salvaguarda contra Morte e recupera o número de Pontos de Vida máximos possíveis de qualquer cura.",
    descricaoCurta: "Escolhe qualquer número de criaturas no alcance: Vantagem em salv. Sabedoria e Salvaguarda contra Morte + cura o máximo possível de qualquer efeito de cura, pela duração (1 min).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    duracao: "Concentração, até 10 minutos",
    descricaoCompleta: "Você toca uma criatura voluntária. Pela duração da magia, o alvo ganha um Deslocamento de Voo de 18 metros e pode pairar. Quando a magia termina, o alvo entra em queda se ainda estiver no alto, a menos que possa impedir a queda. Usando um Espaço de Magia de Círculo Superior. Você pode escolher uma criatura adicional para cada círculo de espaço de magia acima de 3.",
    descricaoCurta: "Toca: Deslocamento de Voo 18m + pairar, pela duração (10 min). Cai se ainda estiver no ar quando acabar (a menos que evite). Upcast: +1 alvo/círculo.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "alvo-por-circulo",
    upcastCirculoBase: 3,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Esconde um baú (até 340L) no Plano Etéreo, junto com uma réplica em miniatura. Ação de Usar Magia tocando a réplica: traz o baú de volta a até 1,5m; ou manda de volta ao Plano Etéreo. Após 60 dias, 5% cumulativo/dia de a magia encerrar sozinha (o baú fica perdido lá dentro se isso acontecer com ele no Plano Etéreo).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Invade os pesadelos do alvo à vista. Salv. Sabedoria ou 4d10 Psíquico + Desvantagem em testes/ataques pela duração (sucesso=metade e encerra). Repete a salvaguarda no fim de cada turno dele (falha=mais dano, sucesso=encerra). Upcast: +1d10.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "dado-por-circulo",
    upcastCirculoBase: 4,
    upcastDado: "1d10",
  ...
  },
  {
    ...
    componentes: "V",
    duracao: "Concentração, até 10 minutos",
    descricaoCompleta: "Uma aura irradia de você em uma Emanação de 9 metros pela duração da magia. Enquanto estiver na aura, você e seus aliados têm Resistência a dano Venenoso e Vantagem nas salvaguardas para evitar ou encerrar efeitos que incluem a condição Amedrontado, Atordoado, Cego, Enfeitiçado, Envenenado, Paralisado ou Surdo.",
    descricaoCurta: "Emanação de 9m: você e aliados dentro têm Resistência a Venenoso + Vantagem em salvaguardas contra/pra encerrar Amedrontado/Atordoado/Cego/Enfeitiçado/Envenenado/Paralisado/Surdo.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Emanação de 9m: você e aliados dentro têm Resistência a Necrótico e PV máximo não pode ser reduzido; aliado a 0 PV que começa turno na aura recupera 1 PV.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Salv. Carisma ou vai pra semiplano inofensivo (Incapacitado lá), pela duração. Se Aberração/Celestial/Elemental/Feérico/Ínfero e durar 1 min completo: vai pro plano de origem em vez de voltar. Upcast: +1 alvo/círculo.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "alvo-por-circulo",
    upcastCirculoBase: 4,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Cubo de até 30m: escolhe 1 efeito (troca por Ação de Usar Magia): Dividir Água (trincheira que se refaz em 1 rodada); Inundação (água sobe 6m, pode virar onda em corpo d'água grande); Redemoinho (puxa criaturas 3m, 2d8 Contundente na entrada, Salv. Força; Atletismo vs sua CD pra nadar pra fora); Redirecionar Fluxo (muda direção da correnteza).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "Ação",
    alcance: "27 metros",
    componentes: "V, S, M (ar, uma pedra, cinzas e água dentro de um frasco incrustado de ouro no valor de 400 ou mais PO)",
    duracao: "Concentração, até 1 hora",
    descricaoCompleta: "Você invoca um Espírito Elemental. Ele se manifesta em um espaço desocupado à sua vista e no alcance da magia e usa o bloco de estatísticas do Espírito Elemental. Ao conjurar a magia, escolha um elemento: Água, Ar, Fogo ou Terra. A criatura se assemelha a uma forma bípede envolta no elemento escolhido, o que determina certas características no bloco de estatísticas da criatura. A criatura desaparece ao ser reduzida a 0 Pontos de Vida ou quando a magia termina. A criatura é uma aliada sua e de seus aliados. Em combate, ela compartilha sua contagem de Iniciativa, mas o turno dela é imediatamente após o seu. Ela obedece aos seus comandos verbais (nenhuma ação é necessária). Se você não emitir nenhum, ela executa a ação Esquivar e usa o movimento dela para evitar o perigo. Usando um Espaço de Magia de Círculo Superior. Use o círculo do espaço de magia para o círculo da magia no bloco de estatísticas. Espíritos invocados por Convocar Familiar assumem formas inspiradas pelos magos que os conjuram. Espírito Elemental Elemental Médio, Neutro CA 11 + o círculo da magia PV 50 + 10 para cada círculo de magia acima de 4 Deslocamento 12 m; Escavação 12 m (apenas Terra); Natação 12 m (apenas Água); Voo 12 m (pairar; apenas Ar) For 18    +4   +4    Des 15    +2   +2    Con 17    +3 Int 4     –3   –3    Sab 10    +0   +0    Car 16    +3   +3 Resistências Ácido (apenas Água), Cortante e Perfurante (apenas Terra), Elétrico e Trovejante (apenas Ar) Imunidades Ígneo (somente Fogo), Venenoso; Envenenado, Exaustão, Paralisado, Petrificado Sentidos Visão no Escuro 18 m, Percepção Passiva 10 Idiomas Primordial, compreende os idiomas que você fala ND Nenhum (XP 0; BP é igual ao seu Bônus de Proficiência) Traços Estado Amorfo (Apenas Água, Ar e Fogo). O espírito pode se mover por um espaço tão estreito quanto 2,5 centímetros de largura sem contá-lo como Terreno Difícil. Ações Ataques Múltiplos. O espírito realiza um número de ataques de Pancada igual à metade do círculo da magia (arredondado para baixo). Pancada. Jogada de Ataque Corpo a Corpo: Bônus é igual ao seu modificador de ataque mágico, alcance 1,5 m. Dano: 1d10 + 4 + o círculo da magia pontos de dano Contundente (apenas Terra), Elétrico (apenas Ar), Gélido (apenas Água) ou Ígneo (apenas Fogo).",
    descricaoCurta: "Invoca Espírito Elemental (Água/Ar/Fogo/Terra à escolha, define resistências/deslocamento). Aliado, compartilha Iniciativa, some a 0 PV/fim. Upcast: círculo do espaço = força.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "formula-propria",
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Cão de guarda fantasmagórico e invisível pra outros, só você vê; late se alguém chegar a 9m sem dizer a senha. Ataca 1x/turno automaticamente: Salv. Destreza ou 4d8 Energético. Move até 9m por Ação de Usar Magia. Dura 8h ou até vocês se separarem por 90m.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Ação Bônus após acertar corpo a corpo/desarmado: +4d6 Psíquico + Salv. Sabedoria ou Atordoado até o fim do seu próximo turno. Upcast: +1d6.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "dado-por-circulo",
    upcastCirculoBase: 4,
    upcastDado: "1d6",
  ...
  },
  {
    ...
    descricaoCurta: "Qualquer criatura à vista: Salv. Sabedoria (Vantagem se já em combate) ou Enfeitiçada/amigável até acabar (1h) ou você/aliados causarem dano nela. Ela sabe que foi enfeitiçada quando acaba. Upcast: +1 alvo/círculo.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "alvo-por-circulo",
    upcastCirculoBase: 4,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "Ação",
    alcance: "9 metros",
    componentes: "V, S, M (uma esfera de vidro)",
    duracao: "Concentração, até 1 minuto",
    descricaoCompleta: "Uma Esfera cintilante envolve uma criatura ou objeto Grande ou menor no alcance da magia. Uma criatura involuntária deve ser bem-sucedida em uma salvaguarda de Destreza ou é envolta pela duração da magia. Nada — nem objetos físicos, energia ou outros efeitos de magia — pode atravessar a barreira, para dentro ou para fora, embora uma criatura na Esfera possa respirar lá. A Esfera é imune a todos os tipos de dano, e uma criatura ou objeto dentro dela não pode ser danificado por ataques ou efeitos provenientes de fora, nem uma criatura dentro da Esfera pode causar dano a qualquer coisa fora dela. A Esfera não tem peso e é grande o suficiente para conter a criatura ou objeto em seu interior. Uma cria- Um tiferino Feiticeiro libera a Esfera Vitriólica em nóticos. tura envolta pode executar uma ação para empurrar-se contra as paredes da Esfera e, assim, rolar a Esfera a até metade do Deslocamento da criatura. Da mesma forma, o globo pode ser pego e movido por outras criaturas. Uma magia Desintegrar que tenha o globo como alvo o destrói sem prejudicar nada em seu interior.",
    descricaoCurta: "Envolve criatura/objeto Grande- numa esfera impenetrável (nada atravessa, imune a todo dano dos dois lados). Salv. Destreza anula se involuntária. Alvo dentro pode rolar a esfera (metade do Deslocamento); outros podem carregá-la. Desintegrar a destrói sem afetar o interior.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "Ação",
    alcance: "45 metros",
    componentes: "V, S, M (uma gota de bile)",
    duracao: "Instantânea",
    descricaoCompleta: "Você aponta para um local no alcance da magia, e uma bola de ácido brilhante de 30 centímetros de diâmetro se espalha lá e explode em uma Esfera de 6 metros de raio. Cada criatura nessa área realiza uma salvaguarda de Destreza. Se falhar, uma criatura sofre 10d4 pontos de dano Ácido e outros 5d4 pontos de dano Ácido no final do próximo turno dela. Em caso de sucesso, uma criatura recebe apenas metade do dano inicial. Usando um Espaço de Magia de Círculo Superior. O dano inicial aumenta em 2d4 para cada círculo de espaço de magia acima de 4.",
    descricaoCurta: "Bola de ácido explode numa esfera de 6m. Salv. Destreza ou 10d4 Ácido + 5d4 Ácido no fim do próximo turno (sucesso=só metade do dano inicial). Upcast: +2d4 no dano inicial.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "dado-por-circulo",
    upcastCirculoBase: 4,
    upcastDado: "2d4",
  ...
  },
  {
    ...
    descricaoCurta: "Converte matéria-prima à vista em objeto do mesmo material (madeira/tecido/corda etc.), até Grande (ou Médio se for metal/pedra/mineral). Sem criaturas/itens mágicos; itens que exigem perícia de Ferramentas de Artesão precisam da proficiência.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "Concentração, até 10 minutos",
    descricaoCompleta: "Você invoca uma aranha, centopeia ou vespa gigante (escolhida ao conjurar a magia). Ela se manifesta em um espaço desocupado à sua vista e no alcance da magia e usa o bloco de estatísticas do Inseto Gigante. A forma escolhida determina certos detalhes no bloco de estatísticas da criatura. A criatura desaparece ao ser reduzida a 0 Pontos de Vida ou quando a magia termina. A criatura é uma aliada sua e de seus aliados. Em combate, ela compartilha sua contagem de Iniciativa, mas o turno dela é imediatamente após o seu. Ele obedece aos seus comandos verbais (nenhuma ação é necessária). Se você não emitir nenhum, ela executa a ação Esquivar e usa o movimento dela para evitar o perigo. Usando um Espaço de Magia de Círculo Superior. Use o círculo do espaço de magia para o círculo da magia no bloco de estatísticas. Inseto Gigante Fera Grande, Sem Alinhamento CA 11 + o círculo da magia PV 30 + 10 para cada círculo de magia acima do 4 Deslocamento 12 m, Escalada 12 m, Voo 12 m (apenas Vespa) For 17     +3   +3    Des 13    +1   +1    Con 15     +2 Int 4      –3   –3    Sab 3     –3   –3    Car 14     +2 Sentidos: Visão no Escuro 18 m, Percepção Passiva 12 Idiomas Compreende os idiomas que você conhece ND Nenhum (XP 0; BP é igual ao seu Bônus de Proficiência) Traços Escalada de Aranha. O inseto pode escalar superfícies difíceis, inclusive ao longo de tetos, sem precisar realizar um teste de atributo. Ações Ataques Múltiplos. O inseto realiza um número de ataques igual à metade do círculo da magia (arredondado para baixo). Punição Venenosa. Jogada de Ataque Corpo a Corpo. Bônus é igual ao seu modificador de ataque mágico, alcance 3 m. Dano: 1d6 + 3 mais o círculo da magia em pontos de dano Perfurante mais 1d4 pontos de dano Venenoso. Raio de Teia (Apenas Aranha). Jogada de Ataque à Distância. Bônus é igual ao seu modificador de ataque mágico, alcance 18 m. Dano: 1d10 + 3 mais o círculo da magia em pontos de dano Contundente, e o Deslocamento do alvo é reduzido a 0 até o início do próximo turno do inseto. Ações Bônus Cuspe Venenoso (Apenas Centopeia). Salvaguarda de Constituição. Sua CD para evitar magia, uma criatura à vista do inseto a até 3 metros. Falha: O alvo tem a condição Envenenado até o início do próximo turno do inseto.",
    descricaoCurta: "Invoca aranha/centopeia/vespa gigante espectral (bloco próprio). Aliada, compartilha Iniciativa, some a 0 PV/fim. Upcast: círculo do espaço = força.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "formula-propria",
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "Concentração, até 1 minuto",
    descricaoCompleta: "Uma criatura que você toca tem a condição Invisível até que a magia termine.",
    descricaoCurta: "Toca: Invisível pela duração (1 min) — não encerra ao atacar/causar dano/conjurar (diferente da Invisibilidade normal).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "Ação",
    alcance: "27 metros",
    componentes: "V, S, M (um tentáculo em conserva e um globo ocular em um frasco incrustado de platina no valor de 400 ou mais PO)",
    duracao: "Concentração, até 1 hora",
    descricaoCompleta: "Você invoca um Espírito Aberrante. Ele se manifesta em um espaço desocupado à sua vista e no alcance da magia e usa o bloco de estatísticas do Espírito Aberrante. Ao conjurar a magia, escolha entre Devorador de Mentes, Pseudo-observador ou Slaad. A criatura se assemelha a uma Aberração desse tipo, o que determina certos detalhes no bloco de estatísticas da criatura. A criatura desaparece ao ser reduzida a 0 Pontos de Vida ou quando a magia termina. A criatura é uma aliada sua e de seus aliados. Em combate, ela compartilha sua contagem de Iniciativa, mas o turno dela é imediatamente após o seu. Ela obedece aos seus comandos verbais (nenhuma ação é necessária). Se você não emitir nenhum, ela executa a ação Esquivar e usa o movimento dela para evitar o perigo. Usando um Espaço de Magia de Círculo Superior. Use o círculo do espaço de magia para o círculo da magia no bloco de estatísticas. Espírito Aberrante Aberração Média, Neutra CA 11 + o círculo da magia PV 40 + 10 para cada círculo de magia acima de 4 Deslocamento 9 m; Voo 9 m (pairar; apenas Pseudoobservador) For 16     +3   +3    Des 10     +0   +0    Con 15     +2   +2 Int 16     +3   +3    Sab 10     +0   +0    Car 6      –2   –2 Imunidades Psíquico Sentidos Visão no Escuro 18 m, Percepção Passiva 10 Idiomas Dialeto Obscuro, entende os idiomas que você fala ND Nenhum (XP 0; BP é igual ao seu Bônus de Proficiência) Traços Aura Sussurrante (Apenas Devorador de Mentes). No início de cada turno do espírito, ele emite energia psiônica se não tem a condição Incapacitado. Salvaguarda de Sabedoria: CD é igual a CD para evitar sua magia, cada criatura (exceto você) a até 1,5 metro do espírito. Falha: 2d6 pontos de dano Psíquico. Regeneração (Apenas Slaad). O espírito recupera 5 Pontos de Vida no início do turno dele se tiver pelo menos 1 Ponto de Vida. Ações Ataques Múltiplos. O espírito realiza um número de ataques igual à metade do círculo da magia (arredondado para baixo). Garras (Apenas Slaad). Jogada de Ataque Corpo a Corpo: Bônus é igual ao seu modificador de ataque mágico, alcance 1,5 m. Dano: 1d10 + 3 mais o círculo da magia em pontos de dano Cortante, e o alvo não pode recuperar Pontos de Vida até o início do próximo turno do espírito. Pancada Psíquica (Apenas Devorador de Mentes). Jogada de Ataque Corpo a Corpo: Bônus é igual ao seu modificador de ataque mágico, alcance 1,5 m. Dano: 1d8 + 3 mais o círculo da magia em pontos de dano Psíquico. Raio Ocular (Somente Pseudo-observador). Jogada de Ataque à Distância: Bônus é igual ao seu modificador de ataque mágico, alcance 45 m. Dano: 1d8 + 3 mais o círculo da magia em pontos de dano Psíquico. Espírito Aberrante (Pseudo-observador)",
    descricaoCurta: "Invoca Espírito Aberrante (Devorador de Mentes/Pseudo-observador/Slaad à escolha, cada um com ataque próprio). Aliado, compartilha Iniciativa, some a 0 PV/fim. Upcast: círculo do espaço = força.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "formula-propria",
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    duracao: "Concentração, até 1 hora",
    descricaoCompleta: "Você invoca o espírito de um Construto. Ele se manifesta em um espaço desocupado à sua vista e no alcance da magia e usa o bloco de estatísticas do Espírito do Constructo. Ao conjurar a magia, escolha um material: Argila, Metal ou Pedra. A criatura se assemelha a uma estátua animada (você determina a aparência) composto do material escolhido, que determina certos detalhes no bloco de estatísticas da criatura. A criatura desaparece ao ser reduzida a 0 Pontos de Vida ou quando a magia termina. A criatura é uma aliada sua e de seus aliados. Em combate, ela compartilha sua contagem de Iniciativa, mas o turno dela é imediatamente após o seu. Ela obedece aos seus comandos verbais (nenhuma ação é necessária). Se você não emitir nenhum, ela executa a ação Esquivar e usa o movimento dela para evitar o perigo. Usando um Espaço de Magia de Círculo Superior. Use o círculo do espaço de magia para o círculo da magia no bloco de estatísticas. Espírito do Constructo Constructo Médio, Neutro CA 13 + o círculo da magia PV 40 + 15 para cada círculo de magia acima de 4 Deslocamento 9 m For 18     +4   +4    Des 10     +0   +0    Con 18    +4    +4 Int 14     +2   +2    Sab 11     +0   +0    Car 5     –3    –3 Resistências Venenoso Imunidades Amedrontado, Enfeitiçado, Envenenado, Exaustão, Paralisado Sentidos Visão no Escuro 18 m, Percepção Passiva 10 Idiomas Compreende os idiomas que você fala ND Nenhum (XP 0; BP é igual ao seu Bônus de Proficiência) Traços Corpo Aquecido (Apenas Metal). Uma criatura que atinge o espírito com um ataque corpo a corpo ou que inicia seu turno imobilizando o espírito sofre 1d10 pontos de dano Ígneo. Letargia Empedernida (Apenas Pedra). Quando uma criatura começa o turno dela a até 3 metros do espírito, o espírito pode atacá-la com energia mágica se o espírito puder vêla. Salvaguarda de Sabedoria: CD é igual a CD para evitar sua magia. Falha: Até o início do próximo turno do espírito, o alvo não pode realizar Ataques de Oportunidade e seu Deslocamento é reduzido pela metade. Ações Ataques Múltiplos. O espírito realiza um número de ataques de Pancada igual à metade do círculo da magia (arredondado para baixo). Pancada. Jogada de Ataque Corpo a Corpo: Bônus é igual ao seu modificador de ataque mágico, alcance 1,5 m. Dano: 1d8 + 4 + o círculo da magia pontos de dano Contundente. Reações Reação Violenta (Apenas Argila). Gatilho: O espírito sofre dano de uma criatura. Reação: O espírito realiza um ataque de Pancada contra essa criatura, se possível, ou o espírito se move até metade do seu Deslocamento em direção a essa criatura sem provocar Ataques de Oportunidade. Espírito do Constructo (Pedra)",
    descricaoCurta: "Invoca Espírito do Constructo (Argila/Metal/Pedra à escolha, cada um com traço próprio — reação violenta, corpo aquecido, letargia). Aliado, compartilha Iniciativa, some a 0 PV/fim. Upcast: círculo do espaço = força.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "formula-propria",
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "Concentração, até 10 minutos",
    descricaoCompleta: "Espírito Dracônico (Frio) Espírito Dracônico Dragão Grande, Neutro CA 14 + o círculo da magia PV 50 + 10 para cada círculo de magia acima de 5 Deslocamento 9 m, Natação 9 m; Voo 18 m For 19    +4    +4    Des 14    +2    +2   Con 17    +3   +3 Int 10    +0    +0    Sab 14    +2    +2   Car 14    +2   +2 Resistências Ácido, Elétrico, Gélido, Ígneo, Venenoso Imunidades Amedrontado, Enfeitiçado, Envenenado Sentidos Visão às Cegas 9 m, Visão no Escuro 18 m, Percepção Passiva 12 Idiomas Dracônico, compreende os idiomas que você fala ND Nenhum (XP 0; BP é igual ao seu Bônus de Proficiência) Traços Resistências Compartilhadas. Ao invocar o espírito, escolha uma das resistências dele. Você tem Resistência ao tipo de dano escolhido até que a magia termine. Ações Ataques Múltiplos. O espírito realiza um número de ataques de Dilacerar igual à metade do círculo da magia (arredondado para baixo) e usa o Ataque de Sopro. Ataque de Sopro. Salvaguarda de Destreza: CD é igual a CD para evitar sua magia, cada criatura em um Cone de 9 metros. Falha: 2d6 pontos de dano de um tipo ao qual o espírito tem Resistência (à sua escolha quando você conjura a magia). Sucesso: Metade do dano. Dilacerar. Jogada de Ataque Corpo a Corpo: Bônus é igual ao seu modificador de ataque mágico, alcance 3 m. Dano: 1d6 + 4 + o círculo da magia pontos de dano Perfurante. Você conjura espíritos dos Planos Elementais que voam ao seu redor em uma Emanação de 4,5 metros pela duração da magia. Até que a magia termine, qualquer ataque que você realizar causa 2d8 pontos de dano adicional quando você atinge uma criatura na Emanação. Este dano é Ácido, Elétrico, Gélido ou Ígneo (à sua escolha quando realizar o ataque). Além disso, o terreno da Emanação é Terreno Difícil para seus inimigos. Usando um Espaço de Magia de Círculo Superior. O dano aumenta em 1d8 para cada círculo de espaço de magia acima de 4.",
    descricaoCurta: "Espíritos elementais voam ao seu redor (Emanação 4,5m); seus ataques causam +2d8 dano (Ácido/Elétrico/Gélido/Ígneo à escolha no momento do ataque) contra quem estiver na Emanação; terreno vira Difícil pra inimigos. Upcast: +1d8.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "dado-por-circulo",
    upcastCirculoBase: 4,
    upcastDado: "1d8",
  ...
  },
  {
    ...
    descricaoCurta: "Espíritos da natureza voam ao seu redor (Emanação 3m). Quando a emanação invade/entra/termina turno perto de criatura à vista: Salv. Sabedoria ou 5d8 Energético (sucesso=metade), 1x/turno. Desengajar vira Ação Bônus pela duração. Upcast: +1d8.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "dado-por-circulo",
    upcastCirculoBase: 4,
    upcastDado: "1d8",
  ...
  },
  {
    ...
    circulo: 4,
    escola: "Abjuração",
    classes: ["Bardo", "Clérigo", "Druida", "Guardião"],
    tempoConjuracao: "Ação",
    alcance: "Toque",
    ...
    descricaoCurta: "Toca: Terreno Difícil não afeta, imune a redução de Deslocamento/Contido/Paralisado por magia, Deslocamento de Natação = normal, escapa de grilhões/Imobilizado gastando 1,5m de movimento, pela duração (1h). Upcast: +1 alvo/círculo.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "alvo-por-circulo",
    upcastCirculoBase: 4,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Parede de fogo (até 18x6m x30cm, ou círculo de 6m diâm.) opaca. Ao surgir: Salv. Destreza ou 5d8 Ígneo (sucesso=metade). 1 lado escolhido causa 5d8 Ígneo a quem terminar turno/entrar a até 3m dele. Upcast: +1d8.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "dado-por-circulo",
    upcastCirculoBase: 4,
    upcastDado: "1d8",
  ...
  },
  {
    ...
    duracao: "Concentração, até 1 hora",
    descricaoCompleta: "Você cria um olho invisível e invulnerável no alcance da magia que fica pairando pela duração. Você recebe mentalmente do olho informações visuais, que pode ver em todas as direções. Ele também tem Visão no Escuro com um alcance de 9 metros. Como uma Ação Bônus, você pode mover o olho até 9 metros em qualquer direção. Uma barreira sólida bloqueia o movimento do olho, mas o olho pode passar por uma abertura de até 2,5 centímetros de diâmetro.",
    descricaoCurta: "Cria olho invisível/invulnerável flutuante; vê em todas as direções (Visão no Escuro 9m) pela duração (1h). Move até 9m por Ação Bônus; passa por frestas de 2,5cm, barreira sólida bloqueia.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "Ação",
    alcance: "Toque",
    componentes: "V, S, M (poeira de diamante no valor de 100 ou mais PO, que a magia consome)",
    duracao: "Concentração, até 1 hora",
    descricaoCompleta: "Até que a magia termine, uma criatura voluntária que você toque tem Resistência a dano Contundente, Cortante e Perfurante.",
    descricaoCurta: "Toca: Resistência a Contundente/Cortante/Perfurante, pela duração (1h).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V",
    duracao: "Instantânea",
    descricaoCompleta: "Você se teleporta para um local no alcance da magia. Você chega exatamente ao local desejado. Pode ser um lugar que você pode ver, um que você pode visualizar ou um que você pode descrever indicando distância e direção, como “60 metros para baixo” ou “90 metros para cima em direção ao noroeste em um ângulo de 45 graus”. Você também pode teleportar uma criatura voluntária. A criatura deve estar a até 1,5 metro de você quando você se teleporta, e ela se teleporta para um espaço a até 1,5 metro do seu espaço de destino. Se você, a outra criatura ou ambas chegarem a um espaço ocupado por uma criatura, ou completamente preenchido por um ou mais objetos, você e qualquer criatura viajando com você sofre 4d6 pontos de dano Energético, e o teleporte falha. Otto, o Bardo, conjura Portal para abrir um portal para as Terras Além.",
    descricaoCurta: "Teleporta você (+1 voluntário a até 1,5m) até 150m, mesmo sem ver o local (pode descrever distância/direção). Se cair em espaço ocupado: 4d6 Energético e falha.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "Ação ou Ritual",
    alcance: "Pessoal",
    componentes: "V, S, M (incenso no valor de 25 ou mais PO, que a magia consome)",
    duracao: "Instantânea",
    descricaoCompleta: "Esta magia coloca você em contato com um deus ou com os servos de um deus. Você faz uma pergunta sobre um objetivo, evento ou atividade específica que ocorrerá em até 7 dias. O Mestre oferece uma resposta verdadeira, que pode ser uma frase curta ou uma rima enigmática. A magia não leva em conta circunstâncias que podem alterar a resposta, como a conjuração de outras magias. Se você conjurar a magia mais de uma vez antes de completar um Descanso Longo, há uma chance cumulativa de 25% para cada conjuração após a primeira de que você não receba resposta.",
    descricaoCurta: "Pergunta sobre evento em até 7 dias; mestre dá resposta verdadeira curta/enigmática, sem considerar fatores que podem mudar depois. Repetir antes de Descanso Longo: 25% cumulativo de falhar.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "Ação",
    alcance: "90 metros",
    componentes: "V, S, M (uma luva)",
    duracao: "Instantânea",
    descricaoCompleta: "Granizo cai em um Cilindro de 6 metros de raio e 12 metros de altura, centrado em um ponto no alcance da magia. Cada criatura no Cilindro realiza uma salvaguarda de Destreza. Se falhar, uma criatura sofre 2d10 pontos de dano Contundente e 4d6 pontos de dano Gélido, ou metade desse dano em caso de sucesso. Pedras de granizo transformam o solo no Cilindro em Terreno Difícil até o final do seu próximo turno. Usando um Espaço de Magia de Círculo Superior. O dano Contundente aumenta em 1d10 para cada círculo de espaço de magia acima de 4. Com sua magia Tempestade Radiante de Jallarzi, a Bruxa Jallarzi faz vrocks se arrependerem de deixar o Abismo.",
    descricaoCurta: "Granizo num cilindro 6m raio x12m altura. Salv. Destreza ou 2d10 Contundente + 4d6 Gélido (sucesso=metade). Terreno vira Difícil até o fim do próximo turno. Upcast: +1d10 Contundente.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "dado-por-circulo",
    upcastCirculoBase: 4,
    upcastDado: "1d10",
  ...
  },
  {
    ...
    descricaoCurta: "Ação Bônus: cria vinha; ataque corpo a corpo (até 9m dela): 4d8 Contundente + puxa até 9m; Enorme- fica Imobilizado (escapa com teste vs sua CD). Só prende 1 de cada vez; libera à vontade. Ação Bônus em turnos seguintes repete o ataque. Upcast: +1 criatura imobilizada simultânea/círculo.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "alvo-por-circulo",
    upcastCirculoBase: 4,
    upcastDado: null,
  ...
  },
  {
    ...
    duracao: "Instantânea",
    descricaoCompleta: "Você libera uma rajada de ar frio. Cada criatura em um Cone de 18 metros originado em você realiza uma salvaguarda de Constituição, sofrendo 8d8 pontos de dano Gélido se falhar, ou metade desse dano em caso de sucesso. Uma criatura morta por esta magia se torna uma estátua congelada até descongelar. Usando um Espaço de Magia de Círculo Superior. O dano aumenta em 1d8 para cada círculo de espaço de magia acima de 5.",
    descricaoCurta: "Cone de 18m. Salv. Constituição ou 8d8 Gélido (sucesso=metade); morte pela magia vira estátua congelada até descongelar. Upcast: +1d8.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "dado-por-circulo",
    upcastCirculoBase: 5,
    upcastDado: "1d8",
  ...
  },
  {
    ...
    alcance: "Toque",
    componentes: "V, S, M (incenso no valor de 1.000 ou mais PO, que a magia consome)",
    duracao: "Até ser dissipada",
    ...
    descricaoCurta: "Esfera de até 18m infundida por 24h. Vigília Consagrada: tipo de criatura escolhido não entra voluntariamente/some efeitos de posse/medo/encanto dele na área. + 1 efeito extra à escolha: Coragem (imune a Amedrontado), Descanso Pacífico (corpos não viram Morto-vivo), Escuridão, Idiomas (comunicação universal), Interferência Extradimensional (bloqueia teleporte), Luz do Dia, Medo, Resistência (a 1 dano), Silêncio, ou Vulnerabilidade (a 1 dano) — pra tipos de criatura à sua escolha.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "Ação",
    alcance: "27 metros",
    componentes: "V, S, M (um relicário no valor de 500 ou mais PO)",
    duracao: "Concentração, até 1 hora",
    descricaoCompleta: "Você invoca um Espírito Celestial. Ele se manifesta em uma forma angelical em um espaço desocupado à sua vista e no alcance da magia e usa o bloco de estatísticas do Espírito Celestial. Ao conjurar a magia, escolha Defensor ou Vingador. Sua escolha determina certos detalhes no bloco de estatísticas da criatura. A criatura desaparece ao ser reduzida a 0 Pontos de Vida ou quando a magia termina. A criatura é uma aliada sua e de seus aliados. Em combate, ela compartilha sua contagem de Iniciativa, mas o turno dela é imediatamente após o seu. Ele obedece aos seus comandos verbais (nenhuma ação é necessária). Se você não emitir nenhum, ela executa a ação Esquivar e usa o movimento dela para evitar o perigo. Usando um Espaço de Magia de Círculo Superior. Use o círculo do espaço de magia para o círculo da magia no bloco de estatísticas. Espírito Celestial Celestial Grande, Neutro CA 11 + o círculo da magia + 2 (apenas Defensor) PV 40 + 10 para cada círculo de magia acima de 5 Deslocamento 9 m, Voo 12 m For 16     +3   +3     Des 14    +2    +2    Con 16     +3 Int 10     +0   +0     Sab 14    +2    +2    Car 16     +3 Resistências Radiante Imunidades Amedrontado, Enfeitiçado Sentidos Visão no Escuro 18 m, Percepção Passiva 12 Idiomas Celestial, compreende os idiomas que você fala ND Nenhum (XP 0; BP é igual ao seu Bônus de Proficiência) Ações Ataques Múltiplos. O espírito realiza um número de ataques igual à metade do círculo da magia (arredondado para baixo). Arco Radiante (Apenas Vingador). Jogada de Ataque à Distância: Bônus é igual ao seu modificador de ataque mágico, alcance 180 m. Dano: 2d6 + 2 + o círculo da magia pontos de dano Radiante. Maça Radiante (Apenas Defensor). Jogada de Ataque Corpo a Corpo: Bônus é igual ao seu modificador de ataque mágico, alcance 1,5 m. Dano: 1d10 + 3 + o círculo da magia pontos de dano Radiante, e o espírito pode escolher a si mesmo ou outra criatura à vista dele a até 3 metros do alvo. A criatura escolhida obtém 1d10 Pontos de Vida Temporários. Toque Curativo (1/Dia). O espírito toca outra criatura. O alvo recupera Pontos de Vida iguais a 2d8 + o círculo da magia. +3 +3                                                          Espírito Celestial (Vingador)",
    descricaoCurta: "Invoca Espírito Celestial (Defensor/Vingador à escolha — cura ou ataque à distância). Aliado, compartilha Iniciativa, some a 0 PV/fim. Upcast: círculo do espaço = força.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "formula-propria",
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    duracao: "Especial",
    descricaoCompleta: "Você puxa mechas de material obscuro do Sombral para criar um objeto no alcance da magia. É um objeto de matéria vegetal (tecido, corda, madeira e algo similar) ou matéria mineral (pedra, cristal, metal e algo similar). O objeto não deve ser maior que um Cubo de 1,5 metro de lado, e deve ter a forma e ser de um material que você já viu. A duração da magia depende do material do objeto, conforme mostrado na tabela Materiais. Se o objeto for composto por vários materiais, use a duração mais curta. Usar qualquer objeto criado por esta magia como componente Material de outra magia faz com que a outra magia falhe. Materiais Materiais                     Duração Matéria vegetal               24 horas Rocha ou cristal              12 horas Metais preciosos              1 hora Pedras preciosas              10 minutos Adamantina ou mitral          1 minuto Usando um Espaço de Magia de Círculo Superior. O Cubo aumenta em 1,5 metro de lado para cada círculo de espaço de magia acima de 5.",
    descricaoCurta: "Cria objeto de material vegetal/mineral (até cubo 1,5m) puxando do Sombral. Duração varia por material (vegetal=24h, rocha/cristal=12h, metal precioso=1h, pedra preciosa=10min, adamantina/mitral=1min). Não pode virar componente de outra magia. Upcast: +1,5m no cubo/círculo.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "flat-por-circulo",
    upcastCirculoBase: 5,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Emanação de 9m: você e aliados têm Vantagem em salvaguardas contra magia/efeitos mágicos; se a salvaguarda daria metade do dano, sucesso = zero dano.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    alcance: "Toque",
    componentes: "V, S, M (uma ágata no valor de 1.000 ou mais PO, que a magia consome)",
    duracao: "Instantânea",
    ...
    descricaoCurta: "8h de ritual + toca 1 Fera/Planta com Int ≤3 (ou planta natural): ganha Int 10 + fala 1 idioma seu (planta vira criatura Planta com sentidos/movimento). Enfeitiçado por 30 dias ou até você/aliado causar dano; depois decide a própria atitude.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Ação Bônus após acertar: +5d10 Energético. Se isso deixa o alvo com 50 PV ou menos: Salv. Carisma ou vai pra semiplano inofensivo (Incapacitado) pela duração (1 min).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "Ação",
    alcance: "Pessoal",
    componentes: "V, S, M (prata e ferro em pó)",
    duracao: "Concentração, até 1 minuto",
    ...
    descricaoCurta: "Celestiais/Elementais/Feéricos/Ínferos/Mortos-vivos têm Desvantagem atacando você, pela duração (1 min). Pode encerrar cedo usando: Exorcizar (manda 1 desses tipos de volta ao plano de origem, Salv. Carisma) ou Quebrar Encantamento (toca aliado possuído/amedrontado/enfeitiçado por eles, remove o efeito).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Humanoide à vista: Salv. Sabedoria (Vantagem se em combate) ou Enfeitiçado pela duração; repete a cada dano sofrido. Vínculo telepático pra comandar (mesmo plano). Upcast: Concentração dura mais (6º:10min / 7º:1h / 8º+:8h).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "outro",
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "Ação",
    alcance: "18 metros",
    componentes: "V, S, M (um objeto com a imagem de um dragão gravado nele no valor de 500 ou mais PO)",
    duracao: "Concentração, até 1 hora",
    descricaoCompleta: "Você invoca um Espírito Dracônico. Ele se manifesta em um espaço desocupado à sua vista e no alcance da magia e usa o bloco de estatísticas do Espírito Dracônico. A criatura desaparece ao ser reduzida a 0 Pontos de Vida ou quando a magia termina. A criatura é uma aliada sua e de seus aliados. Em combate, ela compartilha sua contagem de Iniciativa, mas o turno dela é imediatamente após o seu. Ele obedece aos seus comandos verbais (nenhuma ação é necessária). Se você não emitir nenhum, ela executa a ação Esquivar e usa o movimento dela para evitar o perigo. Usando um Espaço de Magia de Círculo Superior. Use o círculo do espaço de magia para o círculo da magia no bloco de estatísticas.",
    descricaoCurta: "Invoca Espírito Dracônico (Resistência Compartilhada à escolha; ataque de sopro em cone + garras). Aliado, compartilha Iniciativa, some a 0 PV/fim. Upcast: círculo do espaço = força.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "formula-propria",
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Esfera de 6m de névoa amarela, Totalmente Obscurecida, dispersa por vento forte, some do seu lado 3m/turno. Quem entra/começa turno/é atingido pela esfera: Salv. Constituição ou 5d8 Venenoso (sucesso=metade, 1x/turno). Upcast: +1d8.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "dado-por-circulo",
    upcastCirculoBase: 5,
    upcastDado: "1d8",
  ...
  },
  {
    ...
    componentes: "V",
    duracao: "Instantânea",
    descricaoCompleta: "Uma energia destrutiva ondula a partir de você em uma Emanação de 9 metros. Cada criatura à sua escolha na Emanação realiza uma salvaguarda de Constituição. Se falhar, um alvo sofre 5d6 pontos de dano Trovejante e 5d6 pontos de dano Necrótico ou Radiante (à sua escolha) e tem a condição Caído. Em caso de sucesso, um alvo recebe apenas metade do dano.",
    descricaoCurta: "Emanação de 9m ao seu redor. Salv. Constituição (quem você escolher) ou 5d6 Trovejante + 5d6 Necrótico ou Radiante (à sua escolha) + Caído (sucesso=só metade do dano).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "Ação",
    alcance: "Pessoal",
    componentes: "V, S, M (uma tiara em miniatura)",
    duracao: "Concentração, até 1 minuto",
    descricaoCompleta: "Você se envolve com uma majestade sobrenatural em uma Emanação de 3 metros. Sempre que a Emanação entra no espaço de uma criatura à sua vista, ou quando uma criatura à sua vista entra na Emanação ou termina seu turno nela, você pode forçar essa criatura a realizar uma salvaguarda de Sabedoria. Se falhar, o alvo sofre 4d6 pontos de dano Psíquico e tem a condição Caído, e você pode empurrá-lo até 3 metros para longe de você. Em caso de sucesso, o alvo sofre apenas metade do dano. Uma criatura realiza essa salvaguarda apenas uma vez por turno.",
    descricaoCurta: "Emanação de 3m. Quando invade/entra/termina turno perto de criatura à vista: Salv. Sabedoria ou 4d6 Psíquico + Caído + empurra 3m (sucesso=só metade do dano), 1x/turno.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "1 hora",
    alcance: "Toque",
    componentes: "V, S, M (óleos raros no valor de 1.000 ou mais PO, que a magia consome)",
    duracao: "Instantânea",
    descricaoCompleta: "Você toca um Humanoide morto ou parte dele. Se a criatura estiver morta há, no máximo, 10 dias, a magia cria um novo corpo para ela e chama a alma para habitar esse corpo. Jogue 1d10 e consulte a tabela abaixo para determinar a espécie do novo corpo ou o Mestre escolhe outra espécie jogável. 1d10    Espécies                1d10    Espécies 1      Aasimar                   6     Golias 2      Draconato                 7     Pequenino 3      Anão                      8     Humano 4      Elfo                      9     Orc 5      Gnomo                    10     Tiferino A criatura reencarnada faz quaisquer escolhas que a descrição de sua nova espécie oferece e mantém as memórias de sua vida anterior. Ela conserva as capacidades que possuía em sua forma original, mas perde os traços da espécie anterior e adquire os traços da nova espécie.",
    descricaoCurta: "Toca Humanoide morto há até 10 dias: cria novo corpo (1d10 determina espécie ou mestre escolhe) e chama a alma pra ele. Mantém memórias e capacidades; perde traços da espécie antiga, ganha os da nova.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S, M (poeira de diamante no valor de 100 ou mais PO, que a magia consome)",
    duracao: "Instantânea",
    descricaoCompleta: "Você toca uma criatura e magicamente remove um dos seguintes efeitos: • 1 nível de Exaustão • A condição Enfeitiçado ou Petrificado • Uma maldição, incluindo a Sintonização do alvo com um item mágico amaldiçoado • Qualquer redução em um dos valores de atributo do alvo • Qualquer redução nos Pontos de Vida máximos do alvo Um Druida humano conjura Restauração Menor para aliviar o sofrimento de um amigo que foi envenenado em batalha.",
    descricaoCurta: "Toca: remove 1 de: 1 nível de Exaustão; Enfeitiçado ou Petrificado; 1 maldição (inclusive Sintonização amaldiçoada); redução de atributo; ou redução de PV máximo.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "10 minutos",
    alcance: "Pessoal",
    componentes: "V, S, M (um foco no valor de 1.000 ou mais PO, como uma bola de cristal, espelho ou fonte cheia de água)",
    duracao: "Concentração, até 10 minutos",
    descricaoCompleta: "Você pode ver e ouvir uma criatura que você escolheu e que está no mesmo plano de existência que você. O alvo realiza uma salvaguarda de Sabedoria, que é modificada (veja as tabelas abaixo) por quão bem você conhece o alvo e o tipo de conexão física que você tem com ele. O alvo não sabe contra o que está realizando a salvaguarda, apenas que se sente desconfortável. Seu Conhecimento do Alvo É...                Modificador De segunda mão (ouviu falar do alvo)             +5 De primeira mão (conhece o alvo)                 +0 Amplo (conhece bem o alvo)                       –5 Você Possui Algo do Alvo…                    Modificador Retrato ou outra imagem                          –2 Roupa ou outra posse                             –4 Parte do corpo, mecha de cabelo ou               –10 pedaço de unha Em caso de sucesso, o alvo não é afetado e você não pode usar esta magia nele novamente por 24 horas. Se falhar, a magia cria um sensor Invisível e intangível a até 3 metros do alvo. Você pode ver e ouvir através do sensor como se estivesse lá. O sensor se move com o alvo, permanecendo a 3 metros dele pela duração da magia. Se algo puder ver o sensor, ele aparece como uma esfera luminosa do tamanho do seu punho. Em vez de uma criatura, você pode escolher um local que viu. Ao fazer isso, o sensor aparece nesse local e não se move.",
    descricaoCurta: "Vê/ouve 1 criatura conhecida (mesmo plano) ou 1 local já visto. Alvo faz Salv. Sabedoria modificada por quão bem você o conhece e o que você possui dele (retrato −2, posse −4, parte do corpo −10); sucesso = imune por 24h, sem saber o motivo. Falha = sensor invisível a 3m dele, você vê/ouve por ele, pela duração (10 min).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Pede ajuda de uma entidade sobrenatural conhecida (deus, príncipe demônio etc.); ela envia um Celestial/Elemental/Ínfero leal num espaço desocupado. A criatura não é obrigada a nada — negocie um serviço em troca de pagamento (~100 PO/min, 1.000 PO/h, 10.000 PO/dia, ajustável pelo mestre; tarefas alinhadas aos interesses dela custam menos).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Banquete que leva 1h pra ser consumido (até 12 criaturas); quem participa ganha por 24h: Resistência a Venenoso, imunidade a Amedrontado/Envenenado, +2d10 PV máximo (e cura esse tanto).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Parede reta (30x6mx1,5m) ou circular (18m diâm.) de lâminas rodopiantes: Cobertura de ¾, Terreno Difícil. Quem está/entra/termina turno no espaço: Salv. Destreza ou 6d10 Energético (sucesso=metade, 1x/turno).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Caldeirão fixo produz poções (Comuns/Incomuns à escolha, tipo Poção de Cura) — Ação Bônus retira 1; total de poções = seu mod. de atributo (mín 1). Dura 10 min; poções não usadas somem ao reconjurar.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Cura 70 PV + remove Cego/Envenenado/Surdo. Upcast: +10 PV/círculo.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "flat-por-circulo",
    upcastCirculoBase: 6,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Salv. Sabedoria — sucesso: dança comicamente até seu próx. turno (gasta todo movimento dançando); falha: Enfeitiçado pela duração, dança sempre, Desvantagem em Destreza/ataques, outros têm Vantagem contra ele; pode usar ação pra repetir a salvaguarda.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "Ação",
    alcance: "18 metros",
    componentes: "V, S, M (uma pena de cocatriz)",
    duracao: "Concentração, até 1 minuto",
    ...
    descricaoCurta: "Salv. Constituição ou Contido pela duração (falha) / Deslocamento 0 até próx. turno (sucesso); Constructos passam automático. Repete no fim de cada turno: 3 sucessos=encerra, 3 falhas=Petrificado pela duração. Mantendo Concentração a duração toda: Petrificado permanente (só remove com Restauração Maior+).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "1 minuto",
    alcance: "Pessoal",
    componentes: "V, S, M (um conjunto de ferramentas de adivinhação — como cartas ou runas — no valor de 100 ou mais PO)",
    ...
    descricaoCurta: "Sente a rota física mais direta até um local conhecido (mesmo plano, não em movimento); sabe distância/direção e qual caminho seguir em cada bifurcação, pela duração (1 dia).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "Ação",
    alcance: "27 metros",
    componentes: "V, S, M (um frasco com sangue no valor de 600 ou mais PO)",
    duracao: "Concentração, até 1 hora",
    descricaoCompleta: "Você invoca um Espírito Ínfero. Ele se manifesta em um espaço desocupado à sua vista e no alcance da magia e usa o bloco de estatísticas do Espírito Ínfero. Ao conjurar a magia, escolha Demônio, Diabo ou Yugoloth. A criatura se assemelha a um Ínfero do tipo escolhido, o que determina certos detalhes no bloco de estatísticas da criatura. A criatura desaparece ao ser reduzida a 0 Pontos de Vida ou quando a magia termina. A criatura é uma aliada sua e de seus aliados. Em combate, ela compartilha sua contagem de Iniciativa, mas o turno dela é imediatamente após o seu. Ela obedece aos seus comandos verbais (nenhuma ação é necessária). Se você não emitir nenhum, ela executa a ação Esquivar e usa o movimento dela para evitar o perigo. Espírito Bestial Fera Pequena, Neutra CA 11 + o círculo da magia PV 20 (apenas Ar) ou 30 (apenas Água e Terra) + 5 para cada círculo de magia acima de 2 Deslocamento 9 m; Escalada 9 m (Apenas Terra); Natação 9 m (Apenas água); Voo 18 m (Apenas Ar) For 13    +1    +1      Des 16   +3   +3   Con 15    +2 Int 10    +0    +0      Sab 10   +0   +0   Car 16    +3 Sentidos: Visão no Escuro 18 m, Percepção Passiva 12 Idiomas: Compreende os idiomas que você fala ND Nenhum (XP 0; BP é igual ao seu Bônus de Proficiência) Traços Respirar na Água (Apenas Água). O espírito só pode respirar debaixo d’água. Sobrevoo (Apenas Ar). O espírito não provoca Ataques de Oportunidade quando voa para fora do alcance de um inimigo. Táticas de Grupo (Apenas Água e Terra). O espírito tem Vantagem em uma jogada de ataque contra uma criatura se pelo menos um dos aliados do espírito estiver a até 1,5 metro da criatura e o aliado não tem a condição Incapacitado. Ações Ataques Múltiplos. O espírito realiza um número de ataques de Dilacerar igual à metade do círculo da magia (arredondado para baixo). Dilacerar. Jogada de Ataque Corpo a Corpo: Bônus é igual ao seu modificador de ataque mágico, alcance 1,5 m. Dano: 1d8 + 4 mais o círculo da magia em pontos de dano Perfurante. Espírito Ínfero Ínfero Grande, Neutro CA 12 + o círculo da magia PV 50 (apenas Demônio) ou 40 (apenas Diabo) ou 60 (apenas Yugoloth) + 15 para cada círculo de magia acima de 6 Deslocamento 12 m; Escalada 12 m (apenas Demônio); Voo 18 m (apenas Diabo) For 13    +1    +1      Des 16   +3   +3   Con 16    +2    +2 Int 10    +0    +0      Sab 10   +0   +0   Car 16    +3    +3 Resistências Ígneo Imunidades Venenoso; Envenenado Sentidos Visão no Escuro 18 m, Percepção Passiva 10 Idiomas Abissal, Infernal, Telepatia 18 m ND Nenhum (XP 0; BP é igual ao seu Bônus de Proficiência) Traços Espasmos da Morte (Apenas Demônio). Quando o espírito é reduzido a 0 Pontos de Vida ou a magia terminar, o espírito explode. Salvaguarda de Destreza: CD é igual a CD para evitar sua magia, cada criatura em uma Emanação de 3 metros originada a partir do espírito. Falha: 2d10 mais o círculo da magia em pontos de dano Ígneo. Sucesso: Metade do dano. Espírito Ínfero (Diabo) +2 +3 Resistência à Magia. O espírito tem Vantagem em salvaguardas contra magias e outros efeitos mágicos. Visão Diabólica (Apenas Diabo). Escuridão Mágica não impede a Visão no Escuro do espírito. Ações Ataques Múltiplos. O espírito realiza um número de ataques igual à metade do círculo da magia (arredondado para baixo). Arremesso de Chamas (Apenas Diabo). Jogada de Ataque à Distância: Bônus é igual ao seu modificador de ataque mágico, alcance 1,5 m ou alcance 45 m. Dano: 2d6 + 3 mais o círculo da magia em pontos de dano pontos de dano Ígneo. Garras (Apenas Yugoloth). Jogada de Ataque Corpo a Corpo: Bônus é igual ao seu modificador de ataque mágico, alcance 1,5 m. Dano: 1d8 + 3 mais o círculo da magia em pontos de dano Cortante. Imediatamente após o ataque acertar ou errar, o espírito pode se teleportar até 9 metros para um espaço desocupado à vista dele. Mordida (Apenas Demônio). Jogada de Ataque Corpo a Corpo: Bônus é igual ao seu modificador de ataque mágico, alcance 1,5 m. Dano: 1d12 + 3 mais o círculo da magia em pontos de dano Necrótico. Usando um Espaço de Magia de Círculo Superior. Use o círculo do espaço de magia para o círculo da magia no bloco de estatísticas.",
    descricaoCurta: "Invoca Espírito Ínfero (Demônio/Diabo/Yugoloth à escolha, cada um com ataque/traço próprio — explosão ao morrer, arremesso de fogo, garras+teleporte). Aliado, compartilha Iniciativa, some a 0 PV/fim. Upcast: círculo do espaço = força.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "formula-propria",
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "1 minuto ou Ritual",
    alcance: "Toque",
    componentes: "V, S, M (uma safira no valor de 1.000 ou mais PO)",
    duracao: "Até ser dissipada",
    ...
    descricaoCurta: "Marca um objeto (até 3kg, 2m) com uma safira; depois, Ação de Usar Magia + esmagar a safira faz o objeto aparecer na sua mão de qualquer distância/plano (encerra a magia). Se alguém estiver segurando-o, você descobre quem/onde em vez de teleportar.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Seus olhos viram vazio; alvo à vista (até 18m): Salv. Sabedoria ou 1 efeito à escolha: Adoecer (Envenenado), Adormecer (Inconsciente até dano/sacudida), ou Apavorar (Amedrontado, deve Correr pra longe). Ação de Usar Magia em turnos seguintes mira outro alvo (não repete em quem já resistiu).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "Instantânea",
    descricaoCompleta: "Você libera magia virulenta em uma criatura à sua vista e no alcance da magia. O alvo deve realizar uma salvaguarda de Constituição. Se falhar, sofre 14d6 de dano Necrótico e seus Pontos de Vida máximos são reduzidos em um valor igual ao dano sofrido. Em caso de sucesso, sofre apenas metade do dano. Esta magia não pode reduzir os Pontos de Vida máximos de um alvo abaixo de 1. Montaria Fantasmagórica",
    descricaoCurta: "Salv. Constituição ou 14d6 Necrótico + PV máximo reduzido no valor do dano sofrido (não abaixo de 1) (sucesso=metade do dano, sem redução de máximo).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V",
    duracao: "Instantânea",
    descricaoCompleta: "Você e até cinco criaturas voluntárias a até 1,5 metro de você se teleportam instantaneamente para um santuário previamente designado. Você e quaisquer criaturas que se teleportem com você aparecem no espaço desocupado mais próximo do local designado quando você preparou seu santuário (veja abaixo). Se você conjurar esta magia sem primeiro preparar um santuário, a magia não tem efeito. Você deve designar um local, como um templo, como um santuário, conjurando esta magia lá.",
    descricaoCurta: "Você + até 5 voluntários a 1,5m se teleportam pro santuário que você preparou antes (conjurando esta magia lá). Sem santuário preparado, não faz nada.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Cria 2 portais circulares vinculados (1 no alcance, 1 a até 3m de você); o que entra num lado sai do outro como se fossem adjacentes. Ação Bônus muda qual lado está aberto. Dura 10 min (Concentração).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "1 minuto",
    descricaoCompleta: "Esta magia cria um elo mágico entre uma planta inanimada Grande ou maior no alcance da magia e outra planta, a qualquer distância, no mesmo plano de existência. Você deve ter visto ou tocado na planta-alvo pelo menos uma vez antes. Pela duração da magia, qualquer criatura pode entrar na planta-alvo e sair da planta-destino usando 1,5 metro de movimento.",
    descricaoCurta: "Cria elo entre planta Grande+ à vista e outra planta já vista/tocada (qualquer distância, mesmo plano); qualquer um entra numa e sai da outra gastando 1,5m de movimento, por 1 min.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "Ação",
    alcance: "Toque",
    componentes: "V, S, M (cogumelo em pó no valor de 25 ou mais PO, que a magia consome)",
    duracao: "1 hora",
    descricaoCompleta: "Você toca uma criatura voluntária que recebe Visão Verdadeira com um alcance de 36 metros pela duração da magia.",
    descricaoCurta: "Toca: Visão Verdadeira 36m pela duração (1h).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V",
    duracao: "Instantânea",
    descricaoCompleta: "Você pronuncia uma palavra imbuída de poder dos Planos Superiores. Cada criatura à sua escolha no alcance da magia realiza uma salvaguarda de Carisma. Se falhar, um alvo com 50 Pontos de Vida ou menos sofre um efeito com base nos Pontos de Vida atuais dele, conforme mostrado na tabela Efeitos de Palavra Sagrada. Independentemente dos Pontos de Vida, um alvo Celestial, Elemental, Feérico ou Ínfero que falhar na salvaguarda é forçado a voltar ao seu plano de origem (se ainda não estiver lá) e não pode retornar ao plano atual por 24 horas por qualquer meio que não seja uma magia Desejo. Efeitos de Palavra Sagrada Pontos de Vida Efeito 0–20       O alvo morre. 21–30      O alvo está com as condições Atordoado, Cego e Surdo por 1 hora. 31–40      O alvo está com as condições Cego e Surdo por 10 minutos. 41–50      O alvo tem a condição Surdo por 1 minuto.",
    descricaoCurta: "Cada alvo à escolha no alcance: Salv. Carisma; falha + até 50 PV = efeito por PV atual (0-20=morre; 21-30=Atordoado+Cego+Surdo 1h; 31-40=Cego+Surdo 10min; 41-50=Surdo 1min). Celestial/Elemental/Feérico/Ínfero que falha é mandado ao plano de origem por 24h (só Desejo reverte antes).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V",
    duracao: "Instantânea",
    descricaoCompleta: "Você fortalece até seis criaturas à sua vista e no alcance da magia. A magia concede 120 Pontos de Vida Temporários, que você divide entre os alvos da magia.",
    descricaoCurta: "Até 6 criaturas à vista ganham 120 PV temporários no total, divididos como você quiser entre elas.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "Instantânea",
    descricaoCompleta: "Oito raios luminosos partem de você em um Cone de 18 metros. Cada criatura no Cone realiza uma salvaguarda de Destreza. Para cada alvo, jogue 1d8 para determinar qual raio colorido o afeta, consultando a tabela Raios Prismáticos. Raios Prismáticos 1d8 Raio 1   Vermelho. Se falhar: 12d6 pontos de dano Ígneo. Em caso de sucesso: metade do dano. 2   Laranja. Se falhar: 12d6 pontos de dano Ácido. Em caso de sucesso: metade do dano. 3   Amarelo. Se falhar: 12d6 pontos de dano Elétrico. Em caso de sucesso: metade do dano. 4   Verde. Se falhar: 12d6 pontos de dano Venenoso. Em caso de sucesso: metade do dano. 5   Azul. Se falhar: 12d6 pontos de dano Gélido. Em caso de sucesso: metade do dano. 6   Anil. Se falhar: O alvo tem a condição Contido e realiza uma salvaguarda de Constituição no final de cada um dos turnos dele. Se bem-sucedido três vezes, a condição encerra. Se falhar três vezes, ele tem a condição Petrificado até ser liberado por um efeito como a magia Restauração Maior. Os sucessos e fracassos não precisam ser consecutivos; acompanhe ambos até que o alvo alcance três de um tipo. 7   Violeta. Se falhar: O alvo tem a condição Cego e realiza uma salvaguarda de Sabedoria no início do seu próximo turno. Em uma salvaguarda bem-sucedida, a condição encerra. Se falhar, a condição encerra e a criatura se teleporta para outro plano de existência (à escolha do Mestre). 8   Especial. O alvo é atingido por dois raios. Jogue duas vezes, jogando novamente quaisquer 8. Um anão teve um encontro infeliz com o raio anil da Rajada Prismática.",
    descricaoCurta: "Cone de 18m, 8 raios coloridos. Cada alvo: Salv. Destreza, role 1d8 pra ver a cor: 1-Vermelho=12d6 Ígneo; 2-Laranja=12d6 Ácido; 3-Amarelo=12d6 Elétrico; 4-Verde=12d6 Venenoso; 5-Azul=12d6 Gélido (todos sucesso=metade); 6-Anil=Contido, 3 sucessos de Salv. Constituição=livre / 3 falhas=Petrificado; 7-Violeta=Cego, Salv. Sabedoria no próx. turno (sucesso=encerra, falha=teleporta pra outro plano); 8-Especial=2 raios (rerola 8s).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "1 minuto",
    alcance: "Toque",
    componentes: "V, S, M (uma conta de oração)",
    duracao: "1 hora",
    descricaoCompleta: "Uma criatura que você toca recupera 4d8 + 15 Pontos de Vida. Pela duração da magia, o alvo recupera 1 Ponto de Vida no início de cada um dos turnos dele, e quaisquer partes do corpo cortadas voltam a crescer após 2 minutos.",
    descricaoCurta: "Toca: cura 4d8+15 PV + regenera 1 PV no início de cada turno + partes cortadas voltam a crescer em 2 min, pela duração (1h).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "1 hora",
    alcance: "Toque",
    componentes: "V, S, M (um diamante no valor de 1.000 ou mais PO, que a magia consome)",
    duracao: "Instantânea",
    descricaoCompleta: "Com um toque, você revive uma criatura morta há no máximo um século, que não faleceu de velhice nem era um Morto-Vivo. A criatura retorna à vida com todos os seus Pontos de Vida, e a magia neutraliza quaisquer venenos que a afetavam na hora da morte, fecha feridas letais e restaura partes do corpo ausentes. Voltar à vida é um processo difícil. O alvo sofre uma penalidade de −4 em Testes de D20. Sempre que o alvo completar um Descanso Longo, a penalidade é reduzida em 1 até se tornar 0. Conjurar essa magia para reviver uma criatura morta há 365 dias ou mais sobrecarrega você. Até completar um Descanso Longo, você não pode conjurar magias novamente e tem Desvantagem nos Testes de D20.",
    descricaoCurta: "Toca criatura morta há até 1 século (não Morto-vivo, não velhice): revive com todos os PV, remove venenos/feridas fatais, restaura partes ausentes. -4 em Testes de D20 (−1/Descanso Longo). Se morta há 365+ dias: você fica sem conjurar magia + Desvantagem em Testes de D20 até seu próximo Descanso Longo.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "Instantânea",
    descricaoCompleta: "Uma tempestade de fogo aparece no alcance da magia. A área da tempestade consiste em até dez cubos de 3 metros, que você organiza como preferir. Cada Cubo deve ser contíguo a pelo menos um outro Cubo. Cada criatura na área realiza uma salvaguarda de Destreza, sofrendo 7d10 pontos de dano Ígneo se falhar, ou metade desse dano em caso de sucesso. Objetos inflamáveis na área que não estão sendo usados ou carregados entram em combustão.",
    descricaoCurta: "Até 10 cubos de 3m contíguos, formato livre. Salv. Destreza ou 7d10 Ígneo (sucesso=metade); objetos inflamáveis soltos pegam fogo.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "Ação",
    alcance: "Toque",
    componentes: "V, S, M (uma haste de metal bifurcada no valor de 250 ou mais PO e sintonizada a um plano de existência)",
    duracao: "Instantânea",
    descricaoCompleta: "Você e até oito criaturas voluntárias que dão as mãos em um círculo são transportadas para um plano de existência diferente. Você pode especificar um destino-alvo em termos gerais, como a Cidade de Latão no Plano Elemental do Fogo ou o palácio de Dispater no segundo nível dos Nove Infernos, e você aparece dentro ou perto desse destino, conforme determinado pelo Mestre. O Transporte via Plantas e Passo Arbóreo transformam as árvores em uma rede de transporte. Como alternativa, se você souber a sequência de símbolos de um círculo de teleporte em outro plano de existência, esta magia pode levá-lo a esse círculo. Se o círculo de teleporte for pequeno demais para caber todas as criaturas que você transportou, elas aparecem nos espaços desocupados mais próximos ao círculo.",
    descricaoCurta: "Você + até 8 voluntários de mãos dadas viajam pra outro plano (destino geral ou círculo de teleporte conhecido lá); aparecem perto do destino (mestre decide) ou nos espaços mais próximos se o círculo for pequeno demais.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "1 hora",
    alcance: "Toque",
    componentes: "V, S, M (um diamante no valor de 1.000 ou mais PO, que a magia consome, e um recipiente vedável no valor de 2.000 ou mais PO que seja grande o suficiente para conter a criatura sendo clonada)",
    duracao: "Instantânea",
    descricaoCompleta: "Você toca uma criatura ou pelo menos 2,5 centímetros cúbicos da carne dela. Uma duplicata inerte dessa criatura se forma dentro do recipiente usado na conjuração da magia e termina de crescer após 120 dias; você escolhe se o clone finalizado tem a mesma idade da criatura ou é mais nova. O clone permanece inerte e dura indefinidamente enquanto o recipiente o contém permanecer intacto. Se a criatura original morrer após o clone terminar de se formar, a alma da criatura é transferida para o clone se a alma estiver livre e disposta a retornar. O clone é fisicamente idêntico ao original e tem a mesma personalidade, memórias e habilidades, mas nenhum dos equipamentos do original. Os restos originais da criatura, se houver, tornam-se inertes e não podem ser revividos, uma vez que a alma da criatura está em outro lugar.",
    descricaoCurta: "Toca criatura (ou pedaço de carne): cria duplicata inerte num recipiente, pronta em 120 dias, dura indefinidamente enquanto o recipiente intacto. Se o original morrer depois de pronta, a alma migra pro clone (idêntico, mesma personalidade/memórias, sem equipamento); o corpo original vira inerte e não pode mais ser revivido.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S, M (um incenso aceso)",
    duracao: "Concentração, até 8 horas",
    descricaoCompleta: "Você assume o controle do clima a até 8 quilômetros de você pela duração da magia. Você deve estar ao ar livre para conjurar esta magia, e ela encerra se você for para áreas internas. Ao conjurar a magia, você altera a condição climática atual, determinada pelo Mestre. Você pode alterar a precipitação, a temperatura e o vento. Leva 1d4 x 10 minutos para as novas condições do clima entrarem em vigor. Depois que elas se estabelecerem, você pode alterar as condições novamente. Quando a magia termina, o clima volta gradualmente ao normal. Ao alterar as condições meteorológicas, encontre uma condição atual nas tabelas a seguir e altere o está- Um Dragão de Latão conjura Controlar o Clima para salvar uma comunidade de uma tempestade destrutiva. gio em um, para cima ou para baixo. Ao mudar o vento, você pode alterar a direção dele. Precipitação Estágio Condição 1     Limpo 2     Nuvens leves 3     Nublado ou com neblina 4     Chuva, granizo ou neve 5     Chuva torrencial, tempestade de granizo ou nevasca Temperatura                     Vento Estágio Condição                  Estágio Condição 1     Calor insuportável         1      Calmo 2     Quente                     2      Vento moderado 3     Ameno                      3      Vento forte 4     Fresco                     4      Vendaval 5     Frio                       5      Tempestade 6     Frio extremo",
    descricaoCurta: "Controla o clima num raio de 8km (só ao ar livre) pela duração (8h); muda precipitação/temperatura/vento 1 estágio por vez (leva 1d4x10 min pra fazer efeito, pode repetir depois).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Qualquer criatura à vista: Salv. Sabedoria (Vantagem se em combate) ou Enfeitiçada pela duração (1h); repete a cada dano sofrido. Vínculo telepático pra comandar (mesmo plano). Upcast: espaço 9º = Concentração dura até 8h.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: "outro",
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Toca: imunidade a dano Psíquico e Enfeitiçado + imune a detecção de emoção/alinhamento/pensamento/localização e controle mental (nem Desejo afeta), pela duração (24h).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V",
    duracao: "1 hora",
    descricaoCompleta: "Até que a magia termine, ao realizar um teste de Carisma, você pode substituir o número que você tirou por um 15. Além disso, não importa o que você diga, a magia que determinaria se você está dizendo a verdade indica que você está sendo verdadeiro.",
    descricaoCurta: "Testes de Carisma: pode substituir o resultado por 15. Além disso, qualquer magia que detectaria mentira indica que você fala a verdade, não importa o que diga. Dura 1h.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "Ação",
    alcance: "Ilimitado",
    componentes: "V, S, M (um par de anéis de prata interligados)",
    duracao: "24 horas",
    descricaoCompleta: "Você cria um elo telepático entre você e uma criatura voluntária com a qual está familiarizado. A criatura pode estar em qualquer lugar no mesmo plano de existência que você. A magia encerra se você ou o alvo não estiverem mais no mesmo plano. Até que a magia termine, você e o alvo podem compartilhar instantaneamente palavras, imagens, sons e outras mensagens sensoriais entre si através do elo, e o alvo o reconhece como a criatura com a qual está se comunicando. A magia permite que uma criatura compreenda o significado de suas palavras e quaisquer mensagens sensoriais que você enviar a ela.",
    descricaoCurta: "Elo telepático com 1 criatura conhecida voluntária, qualquer distância no mesmo plano; compartilham palavras/imagens/sons instantaneamente, pela duração (24h). Encerra se mudarem de plano.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Salv. Sabedoria (sucesso=imune 24h) ou aprisionado (não precisa respirar/comer/beber, não envelhece, imune a Adivinhação/teleporte) até ser dissipado. Escolha 1 efeito extra: Acorrentar (Contido, preso no lugar), Contenção Reduzida (encolhe e fica numa gema, vê fora mas nada mais passa), Enterrar (globo enterrado, nada passa), Prisão Cercada (semiplano protegido de teleporte/viagem planar), ou Torpor (Inconsciente, não acorda). Você define uma condição rara de encerramento ao conjurar (o mestre valida a probabilidade). Só Dissipar Magia de 9º círculo mirando a prisão/componente encerra.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Distribui até 700 PV de cura entre qualquer nº de criaturas à vista; também remove Cego/Envenenado/Surdo de quem for curado.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V, S",
    duracao: "10 minutos",
    descricaoCompleta: "Uma superfície cintilante e multicolorida forma uma parede vertical opaca de até 27 metros de comprimento, 9 metros de altura e 2,5 centímetros de espessura centrada em um ponto no alcance da magia. Como alternativa, você molda a parede em um globo de até 9 metros de diâmetro centrado em um ponto no alcance da magia. A parede permanece pela duração. Se você posicionar a parede em um espaço ocupado por uma criatura, a magia encerra instantaneamente sem efeito. A parede emite Luz Plena por 30 metros e Meia-luz por mais 30 metros. Você e as criaturas que você escolher ao conjurar a magia podem passar e ficar perto da muralha sem sofrer dano. Se outra criatura que possa ver a parede se mover a até 6 metros dela ou começar o turno nela, a criatura deve ser bem-sucedida em uma salvaguarda de Constituição ou tem a condição Cego por 1 minuto. A parede possui sete camadas, cada uma de uma cor diferente. Uma criatura que atravessa ou entra na parede passa uma camada por vez e deve realizar uma salvaguarda de Destreza ou é afetada pelas propriedades da camada, conforme descrito na tabela Camadas Prismáticas. A parede tem CA 10 e pode ser destruída uma camada por vez, em ordem do vermelho ao violeta, por meios específicos para cada camada. Se uma camada for destruída, ela desaparece pela duração da magia. Campo Antimagia não tem efeito na parede, e Dissipar Magia pode afetar apenas a camada violeta. Camadas Prismáticas Ordem Efeitos 1   Vermelho. Se falhar: 12d6 pontos de dano Ígneo. Em caso de sucesso: metade do dano. Efeitos Adicionais: Ataques à distância não mágicos não podem atravessar esta camada, que é destruída se sofrer ao menos 25 pontos de dano Gélido. 2   Laranja. Se falhar: 12d6 pontos de dano Ácido. Em caso de sucesso: metade do dano. Efeitos Adicionais: Ataques mágicos à distância não podem passar por esta camada, que é destruída por um vento forte (como o criado por Lufada de Vento). 3   Amarelo. Se falhar: 12d6 pontos de dano Elétrico. Em caso de sucesso: metade do dano. Efeitos Adicionais: A camada é destruída se sofrer ao menos 60 pontos de dano Energético. 4   Verde. Se falhar: 12d6 pontos de dano Venenoso. Em caso de sucesso: metade do dano. Efeitos Adicionais: Uma magia Criar Passagem ou outra magia de círculo igual ou superior, que possa abrir um portal em uma superfície sólida, destrói esta camada. 5   Azul. Se falhar: 12d6 pontos de dano Gélido. Em caso de sucesso: metade do dano. Efeitos Adicionais: A camada é destruída se sofrer ao menos 25 pontos de dano Ígneo. 6   Anil. Se falhar: O alvo tem a condição Contido e realiza uma salvaguarda de Constituição no final de cada um dos turnos dele. Se bem-sucedido três vezes, a condição encerra. Se falhar três vezes, ele tem a condição Petrificado até ser liberado por um efeito como a magia Restauração Maior. Os sucessos e fracassos não precisam ser consecutivos; acompanhe ambos até que o alvo alcance três de um tipo. Efeitos Adicionais: Magias não podem ser conjuradas através desta camada, que é destruída pela Luz Plena conjurada pela magia Luz do Dia. Ordem Efeitos 7    Violeta. Se falhar: O alvo tem a condição Cego e realiza uma salvaguarda de Sabedoria no início do seu próximo turno. Em caso de sucesso a condição encerra. Se falhar, a condição encerra e a criatura se teleporta para outro plano de existência (à escolha do Mestre). Efeitos Adicionais: Esta camada é destruída por Dissipar Magia.",
    descricaoCurta: "Parede opaca multicor (até 27×9m×2,5cm) ou globo (9m diâm.), 10 min. Não pode sobrepor criatura (magia falha). Luz Plena 30m + Meia-luz 30m. Você e aliados escolhidos passam sem dano. Outras criaturas que cheguem a 6m/comecem turno lá: Salv. Constituição ou Cego 1 min. 7 camadas (atravessar = 1 Salv. Destreza por camada): 1.Vermelho: 12d6 Ígneo(sucesso=metade); bloqueia dist. não-mágico; destrói c/ 25+ dano Gélido. 2.Laranja: 12d6 Ácido; bloqueia dist. mágico; destrói c/ vento forte. 3.Amarelo: 12d6 Elétrico; destrói c/ 60+ dano Energético. 4.Verde: 12d6 Venenoso; destrói c/ Criar Passagem (círculo igual+). 5.Azul: 12d6 Gélido; destrói c/ 25+ dano Ígneo. 6.Anil: Contido, Salv. Con no fim de cada turno (3 sucessos=livre / 3 falhas=Petrificado); bloqueia magia; destrói c/ Luz do Dia. 7.Violeta: Cego + Salv. Sab no próx. turno (sucesso=acaba / falha=teleporta p/ outro plano); destrói c/ Dissipar Magia. CA10; destrói-se camada por camada (vermelho→violeta) pelo método específico. Campo Antimagia não afeta; Dissipar Magia só afeta a violeta.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    componentes: "V",
    duracao: "Instantânea",
    descricaoCompleta: "Você obriga uma criatura à sua vista e no alcance da magia a morrer. Se o alvo tiver 100 Pontos de Vida ou menos, ele morre. Caso contrário, sofre 12d12 pontos de dano Psíquico.",
    descricaoCurta: "Alvo com até 100 PV morre instantaneamente; acima disso, sofre 12d12 Psíquico. Sem salvaguarda.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    descricaoCurta: "Cura todos os PV do alvo + remove Amedrontado/Atordoado/Enfeitiçado/Envenenado/Paralisado; se Caído, pode usar a Reação pra levantar.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    tempoConjuracao: "1 hora",
    alcance: "Toque",
    componentes: "V, S, M (diamantes no valor de 25.000 ou mais PO, que a magia consome)",
    duracao: "Instantânea",
    descricaoCompleta: "Você toca uma criatura morta há no máximo 200 anos e que morreu por qualquer motivo, exceto velhice. A criatura retorna à vida com todos os seus Pontos de Vida. Esta magia fecha todas as feridas, neutraliza qualquer veneno, cura todos os contágios mágicos e elimina quaisquer maldições afetando a criatura quando ela morreu. A magia substitui órgãos e membros danificados ou ausentes. Se a criatura era Morto-Vivo, ela é restaurada à sua forma não Morta-Viva. A magia pode criar um novo corpo se o original não existir mais; nesse caso, você deve pronunciar o nome da criatura. A criatura então aparece em um espaço desocupado à sua escolha, a até 3 metros de você.",
    descricaoCurta: "Toca criatura morta há até 200 anos (não velhice): revive com todos os PV, remove venenos/maldições/contágios, restaura partes ausentes, reverte Morto-vivo. Se o corpo não existir mais, cria um novo dizendo o nome dela (aparece a até 3m de você).",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
  ...
  },
  {
    ...
    duracao: "8 horas",
    descricaoCompleta: "Você toca uma criatura voluntária e concede a ela a capacidade de ver o futuro imediato. Pela duração da magia, o alvo tem Vantagem em Testes de D20, enquanto outras criaturas têm Desvantagem em jogadas de ataque contra ele. A magia se encerra se você a conjurar novamente.",
    descricaoCurta: "Toca: Vantagem em Testes de D20 + outros têm Desvantagem atacando o alvo, pela duração (8h). Encerra se reconjurada.",
    fonte: "Livro do Jogador (D&D 5e 2024)",
    upcastTipo: null,
    upcastCirculoBase: null,
    upcastDado: null,
